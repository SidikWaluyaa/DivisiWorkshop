                ...
                <div class="flex flex-col gap-5 sm:gap-8">
                    <div class="flex items-center justify-between">
                        ...
                        <div class="flex bg-gray-100 p-1 rounded-2xl border border-gray-200 shadow-inner">
                            <a href="{{ route('finance.cs-verification') }}" class="px-6 py-2.5 text-xs font-black uppercase tracking-widest text-gray-400 hover:text-gray-900 transition-all italic">Pending</a>
                            <a href="{{ route('finance.cs-verification.history') }}" class="px-6 py-2.5 text-xs font-black uppercase tracking-widest bg-white text-orange-600 rounded-xl shadow-md border border-orange-100 italic">History</a>
                        </div>
                    </div>

                    {{-- Search Form --}}
                    <form action="{{ route('finance.cs-verification.history') }}" method="GET" class="flex items-center gap-3">
                                ...
                                <tr class="hover:bg-orange-50/30 transition-colors group">
                                    <td class="px-8 py-6">
                                        <div class="flex flex-col">
                                            <span class="text-sm font-black text-gray-900 italic tracking-tighter">{{ $payment->updated_at->format('d M Y') }}</span>
                                            <span class="text-[10px] font-bold text-gray-400 uppercase tracking-widest mt-0.5">{{ $payment->updated_at->format('H:i') }} WIB</span>
                                        </div>
                                    </td>
                                    <td class="px-8 py-6">
                                        <div class="flex flex-col gap-1">
                                            <a href="{{ route('finance.show', $payment->work_order_id) }}" class="text-xs font-black text-[#1B8A68] hover:underline uppercase italic tracking-tight">{{ $payment->spk_number_snapshot }}</a>
                                            <span class="text-[11px] font-bold text-gray-500 italic">{{ $payment->customer_name_snapshot }}</span>
                                        </div>
                                    </td>
                                    <td class="px-8 py-6 text-right">
                                        <span class="text-lg font-black text-gray-900 italic tracking-tighter">Rp {{ number_format($payment->amount_total, 0, ',', '.') }}</span>
                                    </td>
                                    <td class="px-8 py-6">
                                        <span class="px-2.5 py-1 bg-gray-100 text-gray-600 text-[9px] font-black rounded-lg uppercase tracking-widest italic border border-gray-200">{{ $payment->payment_method }}</span>
                                    </td>
                                    <td class="px-8 py-6">
                                        <p class="text-[11px] text-gray-500 font-medium italic line-clamp-2 max-w-xs">"{{ $payment->notes }}"</p>
                                    </td>
