    ...
    <div class="min-h-screen bg-[#F8FAFC]">
        ...
        <div class="bg-white/90 shadow-2xl border-b border-gray-100 sticky top-0 z-40 backdrop-blur-xl">
            <div class="max-w-4xl mx-auto px-6 py-8">
                <div class="flex items-center gap-6">
                    <a href="{{ route('finance.payments.index') }}" class="w-12 h-12 rounded-full bg-gray-50 border-2 border-gray-100 text-gray-400 hover:text-[#1B8A68] hover:border-[#1B8A68]/30 hover:shadow-lg transition-all flex items-center justify-center">
        ...
        <div class="max-w-4xl mx-auto px-6 py-12">
            <div class="bg-white rounded-[3rem] shadow-2xl border border-gray-100 overflow-hidden relative">
                ...
                <form action="{{ route('finance.payments.store') }}" method="POST" class="relative z-10 p-10 space-y-8">
                    @csrf
                        ...
                        <select name="invoice_id" id="invoice_id" required class="w-full px-6 py-5 bg-gray-50 border-2 border-transparent rounded-2xl focus:bg-white focus:border-[#1B8A68]/20 focus:ring-4 focus:ring-[#1B8A68]/5 text-sm font-black italic tracking-tight text-gray-700 transition-all duration-500 shadow-inner outline-none appearance-none">
                            ...
                            @foreach($invoices as $invoice)
                                <option value="{{ $invoice->id }}" {{ old('invoice_id') == $invoice->id ? 'selected' : '' }}>
                                    {{ $invoice->invoice_number }} — {{ $invoice->customer->name ?? 'N/A' }} — Sisa: Rp {{ number_format($invoice->remaining_balance, 0, ',', '.') }}
                                </option>
                            @endforeach
                        </select>
                        @error('invoice_id') <p class="text-red-500 text-xs font-bold mt-2 italic">{{ $message }}</p> @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="number" name="amount" id="amount" value="{{ old('amount') }}" required min="1" step="1" placeholder="Masukkan nominal..." class="w-full px-6 py-5 bg-gray-50 border-2 border-transparent rounded-2xl focus:bg-white focus:border-[#1B8A68]/20 focus:ring-4 focus:ring-[#1B8A68]/5 text-xl font-black italic tracking-tight text-gray-900 transition-all duration-500 shadow-inner outline-none tabular-nums placeholder-gray-300">
                        @error('amount') <p class="text-red-500 text-xs font-bold mt-2 italic">{{ $message }}</p> @enderror
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <input type="date" name="payment_date" id="payment_date" value="{{ old('payment_date', date('Y-m-d')) }}" required class="w-full px-6 py-5 bg-gray-50 border-2 border-transparent rounded-2xl focus:bg-white focus:border-[#1B8A68]/20 focus:ring-4 focus:ring-[#1B8A68]/5 text-sm font-black italic tracking-tight text-gray-700 transition-all duration-500 shadow-inner outline-none">
                            @error('payment_date') <p class="text-red-500 text-xs font-bold mt-2 italic">{{ $message }}</p> @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <div class="relative">
                                <select name="payment_method" id="payment_method" required class="w-full px-6 py-5 bg-gray-50 border-2 border-transparent rounded-2xl focus:bg-white focus:border-[#1B8A68]/20 focus:ring-4 focus:ring-[#1B8A68]/5 text-sm font-black italic tracking-tight text-gray-700 transition-all duration-500 shadow-inner outline-none appearance-none">
                                    ...
                                    <option value="TUNAI" {{ old('payment_method') == 'TUNAI' ? 'selected' : '' }}>Tunai / Cash (Otomatis Verified)</option>
                                    <option value="EDC" {{ old('payment_method') == 'EDC' ? 'selected' : '' }}>Mesin EDC</option>
                                </select>
                            ...
                            @error('payment_method') <p class="text-red-500 text-xs font-bold mt-2 italic">{{ $message }}</p> @enderror
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <textarea name="notes" id="notes" rows="3" placeholder="Contoh: DP via BCA, Transfer Mandiri, dsb..." class="w-full px-6 py-5 bg-gray-50 border-2 border-transparent rounded-2xl focus:bg-white focus:border-[#1B8A68]/20 focus:ring-4 focus:ring-[#1B8A68]/5 text-sm font-black italic tracking-tight text-gray-700 transition-all duration-500 shadow-inner outline-none resize-none placeholder-gray-300">{{ old('notes') }}</textarea>
                        @error('notes') <p class="text-red-500 text-xs font-bold mt-2 italic">{{ $message }}</p> @enderror
                    </div>

                    {{-- Submit --}}
                    <div class="flex items-center justify-end gap-4 pt-4">
                        <a href="{{ route('finance.payments.index') }}" class="px-8 py-4 bg-gray-100 hover:bg-gray-200 text-gray-600 rounded-[2rem] font-black text-xs uppercase tracking-[0.2em] italic transition-all">Batal</a>
