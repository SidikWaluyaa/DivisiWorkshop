<x-app-layout>
    <div class="min-h-screen bg-[#F8FAFC]">
        ...
        {{-- Filters & Search --}}
        <div class="max-w-7xl mx-auto px-6 pt-6">
            <form action="{{ route('finance.verifications.index') }}" method="GET" class="flex flex-col md:flex-row items-center justify-between gap-4">
                {{-- Legend & Match Type Filter --}}
                <div class="flex flex-wrap items-center gap-4 w-full md:w-auto">
                    <select name="match_type" onchange="this.form.submit()" class="px-5 py-4 bg-white border-2 border-gray-100 rounded-xl focus:border-purple-500/30 focus:ring-4 focus:ring-purple-500/10 text-[11px] font-black uppercase tracking-wider italic text-gray-600 transition-all duration-300 shadow-sm outline-none appearance-none cursor-pointer">
                        ...
                        <option value="exact" {{ request('match_type') === 'exact' ? 'selected' : '' }}>🟣 Exact Match (Auto)</option>
                        <option value="partial" {{ request('match_type') === 'partial' ? 'selected' : '' }}>🟡 Partial Match (Manual)</option>
        ...
        <div class="max-w-7xl mx-auto px-6 py-6">
            <div class="space-y-6">
                @forelse($candidates as $candidate)
                    ...
                    @endphp
                    <div class="bg-white rounded-[2.5rem] p-8 border border-gray-100 border-l-4 {{ $borderColor }} shadow-2xl overflow-hidden relative">
                        <div class="flex flex-col lg:flex-row gap-8">
                            ...
                            <div class="flex-1">
                                ...
                                @elseif($matchType === 'partial')
                                    ...
                                    <form action="{{ route('finance.verifications.verify', $payment->id) }}" method="POST" onsubmit="return confirm('Nominal berbeda! Yakin ingin verifikasi dengan mutasi ini? Selisih akan dicatat di log.')">
                                        @csrf
                                        <div class="mb-4">
                                            ...
                                            <select name="mutation_id" required class="w-full px-5 py-4 bg-white border-2 border-amber-200 rounded-xl text-sm font-bold italic tracking-tight text-gray-700 focus:border-amber-400 focus:ring-2 focus:ring-amber-100 outline-none appearance-none">
                                                ...
                                                @foreach($partialMutations as $pm)
                                                    ...
                                                    <option value="{{ $pm->id }}">
                                                        ...
                                                        @if($selisih > 0) (selisih Rp {{ number_format($selisih, 0, ',', '.') }}) @endif
                                                    </option>
                                                @endforeach
                                            </select>
                                        </div>
                                ...
                                @else
                                    ...
                                    <div class="p-6 bg-gray-50 rounded-2xl border-2 border-gray-200 h-full flex flex-col items-center justify-center text-center">
                                        ...
                                        <p class="text-[10px] text-gray-400 italic font-bold leading-relaxed max-w-[250px]">Belum ada data mutasi bank dengan nomor invoice <strong class="text-gray-600">{{ $payment->invoice->invoice_number ?? '-' }}</strong>. Import mutasi terlebih dahulu.</p>
                                    </div>
                                @endif
                            </div>
                        </div>
