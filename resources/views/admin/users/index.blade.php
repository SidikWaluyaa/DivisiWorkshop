<x-app-layout>
    ...
    <x-modal name="create-user-modal" :show="$errors->any() && old('form_type') === 'create_user'" focusable maxWidth="4xl">
        <form method="POST" action="{{ route('admin.users.store') }}" x-data="{ role: 'user' }" onsubmit="let btn = this.querySelector('button[type=submit]'); setTimeout(() => btn.disabled = true, 0); btn.querySelector('.submit-spinner').classList.remove('hidden'); btn.querySelector('.submit-text').innerText = '{{ __('Menyimpan...') }}';">
            @csrf
            ...
            <div class="p-6 bg-gray-50/50 dark:bg-gray-800 border-t border-gray-100 dark:border-gray-700 flex justify-end gap-3 rounded-b-lg">
                ...
                <button type="submit" 
                    ...
                    <span class="submit-text">{{ __('Simpan User') }}</span>
                </button>
            </div>
        </form>
    </x-modal>
