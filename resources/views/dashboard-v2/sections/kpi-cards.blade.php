{{-- KPI Metrics — 8 cards matching CX/Warehouse dashboard style --}}
<section class="animate-fade-in-up delay-100">
    <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-4 gap-4">
        ...
        <div x-data="{ open: false }" class="stat-card group bg-gradient-to-br from-teal-500 to-teal-600 rounded-2xl p-5 shadow-lg border border-teal-400/30 !overflow-visible relative transition-all duration-300"
            ...
            <div id="kpi-cs-leads" class="text-3xl font-black text-white mb-1">{{ $kpi['cs']['leads'] ?? 0 }}</div>
            <div class="text-xs font-bold text-white/70 uppercase tracking-wider">Total Leads</div>
            <div class="mt-2 flex items-center gap-2">
                <span id="kpi-cs-delta" class="px-2 py-0.5 rounded-md text-[10px] font-bold {{ ($kpi['cs']['leads_delta'] ?? 0) > 0 ? 'bg-white/20 text-white' : (($kpi['cs']['leads_delta'] ?? 0) < 0 ? 'bg-white/20 text-white' : 'bg-white/10 text-white/60') }}">
                    {{ ($kpi['cs']['leads_delta'] ?? 0) > 0 ? '+' : '' }}{{ $kpi['cs']['leads_delta'] ?? 0 }}%
                </span>
        ...
        <div x-data="{ open: false }" class="stat-card group bg-gradient-to-br from-green-500 to-emerald-600 rounded-2xl p-5 shadow-lg border border-emerald-400/30 !overflow-visible relative transition-all duration-300"
            ...
            <div id="kpi-cs-conversion" class="text-3xl font-black text-white mb-1">{{ $kpi['cs']['conversion'] ?? 0 }}%</div>
            ...
            <div class="mt-4 w-full bg-white/20 rounded-full h-1.5 overflow-hidden shadow-inner">
                <div id="kpi-cs-conversion-progress" class="bg-white h-1.5 rounded-full shadow-[0_0_10px_rgba(255,255,255,0.5)] transition-all duration-1000" style="width: {{ $kpi['cs']['conversion'] ?? 0 }}%"></div>
            </div>
            <div class="mt-2 text-[10px] text-white/60 font-medium">
                <span id="kpi-cs-closings">{{ $kpi['cs']['closings'] ?? 0 }}</span> closing dari {{ $kpi['cs']['leads'] ?? 0 }} leads
            </div>
        </div>
        ...
        <div x-data="{ open: false }" class="stat-card group bg-gradient-to-br from-orange-500 to-orange-600 rounded-2xl p-5 shadow-lg border border-orange-400/30 !overflow-visible relative transition-all duration-300"
            ...
            <div id="kpi-ws-active" class="text-3xl font-black text-white mb-1">{{ $kpi['workshop']['active'] ?? 0 }}</div>
            ...
            <div class="mt-4 flex items-center gap-2">
                ...
                <div id="kpi-ws-overdue" class="text-[10px] font-black text-white px-1.5 py-0.5 bg-white/20 rounded">
                    <span class="count-text">{{ $kpi['workshop']['overdue'] ?? 0 }}</span>
                    <span class="opacity-50 mx-0.5">/</span>
                    <span id="kpi-ws-normal">{{ $kpi['workshop']['active'] ?? 0 }}</span>
                </div>
            </div>
        </div>

        {{-- Workshop: SPK Selesai --}}
        <div x-data="{ open: false }" class="stat-card group bg-gradient-to-br from-amber-400 to-orange-500 rounded-2xl p-5 shadow-lg border border-amber-300/30 !overflow-visible relative transition-all duration-300"
            ...
            <div id="kpi-ws-completed" class="text-3xl font-black text-white mb-1">✓ {{ $kpi['workshop']['completed'] ?? 0 }}</div>
            <div class="text-xs font-bold text-white/70 uppercase tracking-wider">SPK Selesai</div>
            <div class="mt-2 flex items-center justify-between">
                <div class="text-[10px] text-white/60 font-medium">Growth <span id="kpi-ws-delta" class="font-bold">({{ ($kpi['workshop']['completed_delta'] ?? 0) > 0 ? '+' : '' }}{{ $kpi['workshop']['completed_delta'] ?? 0 }}%)</span></div>
        ...
        <div x-data="{ open: false }" class="stat-card group bg-gradient-to-br from-indigo-500 to-blue-600 rounded-2xl p-5 shadow-lg border border-indigo-400/30 !overflow-visible relative transition-all duration-300"
            ...
            <div id="kpi-gd-value" class="text-3xl font-black text-white mb-1">Rp {{ number_format(($kpi['gudang']['inventory_value'] ?? 0) / 1000000, 1, ',', '.') }}jt</div>
            ...
            <div class="mt-4 flex items-center gap-2">
                ...
                <div id="kpi-gd-stored" class="text-[11px] font-bold text-white/80">
                    📍 {{ $kpi['gudang']['stored_items'] ?? 0 }} Item Tersimpan
                </div>
            </div>
        </div>
        ...
        <div x-data="{ open: false }" class="stat-card group bg-white rounded-2xl p-5 shadow-lg border border-gray-100 hover:border-red-200 !overflow-visible relative transition-all duration-300"
            ...
            <div id="kpi-gd-lowstock-val" class="text-3xl font-black text-gray-800 mb-1 {{ ($kpi['gudang']['low_stock'] ?? 0) > 0 ? 'text-red-600' : '' }}">{{ $kpi['gudang']['low_stock'] ?? 0 }}</div>
            <div class="text-xs font-bold text-gray-400 uppercase tracking-wider">Stok Kritis</div>
            <div class="mt-4 flex items-center gap-2">
                <div id="kpi-gd-lowstock" class="px-2 py-0.5 rounded-md bg-red-100 text-red-600 text-[10px] font-bold {{ ($kpi['gudang']['low_stock'] ?? 0) > 0 ? 'animate-pulse' : 'hidden' }}">
                    <span class="count-text">🔻 {{ $kpi['gudang']['low_stock'] ?? 0 }}</span> Needs Restock
                </div>
                @if(($kpi['gudang']['low_stock'] ?? 0) <= 0)
        ...
        <div x-data="{ open: false }" class="stat-card group bg-gradient-to-br from-purple-500 to-fuchsia-600 rounded-2xl p-5 shadow-lg border border-purple-400/30 !overflow-visible relative transition-all duration-300"
            ...
            <div id="kpi-cx-rate" class="text-3xl font-black text-white mb-1">{{ $kpi['cx']['resolution_rate'] ?? 0 }}%</div>
            <div class="text-xs font-bold text-white/70 uppercase tracking-wider">Resolution Rate</div>
            <div class="mt-4 w-full bg-white/20 rounded-full h-1.5 overflow-hidden shadow-inner">
                <div id="kpi-cx-resolution-progress" class="bg-white h-1.5 rounded-full shadow-[0_0_10px_rgba(255,255,255,0.5)] transition-all duration-1000" style="width: {{ $kpi['cx']['resolution_rate'] ?? 0 }}%"></div>
            </div>
            <div id="kpi-cx-delta" class="hidden mt-2"></div>
        </div>

        {{-- CX: Open Issues --}}
        <div x-data="{ open: false }" class="stat-card group bg-white rounded-2xl p-5 shadow-lg border border-red-100 hover:border-red-200 !overflow-visible relative transition-all duration-300 {{ ($kpi['cx']['open_issues'] ?? 0) > 0 ? 'urgent-glow' : '' }}"
             :class="{ 'z-[50] scale-[1.02]': open, 'hover:z-[40]': !open }">
            <div class="flex items-center justify-between mb-3">
                <div class="w-12 h-12 bg-red-50 rounded-xl flex items-center justify-center group-hover:scale-110 transition-transform shadow-sm">
                    <svg class="w-6 h-6 text-red-500 {{ ($kpi['cx']['open_issues'] ?? 0) > 0 ? 'animate-pulse' : '' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"></path></svg>
                </div>
                <div class="relative">
                    <button @click.stop="open = !open" class="p-1 text-gray-400 hover:text-gray-600 transition-colors cursor-pointer outline-none">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
            ...
            <div id="kpi-cx-open" class="text-3xl font-black text-gray-800 mb-1 {{ ($kpi['cx']['open_issues'] ?? 0) > 0 ? 'text-red-600' : '' }}">{{ $kpi['cx']['open_issues'] ?? 0 }}</div>
            <div class="text-xs font-bold text-gray-400 uppercase tracking-wider">CX Open Issues</div>
            <div class="mt-4 flex items-center gap-2">
                <span id="kpi-cx-avgtime" class="text-[10px] text-gray-400 font-medium italic">Avg Response: {{ $kpi['cx']['avg_response'] ?? 0 }}h</span>
            </div>
        </div>
