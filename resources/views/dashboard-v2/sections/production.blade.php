...
<section class="grid grid-cols-1 lg:grid-cols-3 gap-6 animate-fade-in-up delay-300">
    ...
    <div class="bg-white rounded-3xl p-6 shadow-xl border border-gray-100">
        ...
        <div id="technician-list" class="space-y-3">
            @forelse($production['technicians'] as $index => $tech)
            <div class="flex items-center gap-3 p-3.5 rounded-2xl {{ $index === 0 ? 'bg-gradient-to-r from-[#FFC232]/10 to-[#FFC232]/5 border border-[#FFC232]/20' : 'bg-gray-50/50 border border-gray-100' }} hover:shadow-md transition-all group">
                <div class="w-10 h-10 rounded-xl flex items-center justify-center text-sm font-black flex-shrink-0 shadow-sm
                    {{ $index === 0 ? 'bg-gradient-to-br from-[#FFC232] to-orange-400 text-white' :
                       ($index === 1 ? 'bg-gradient-to-br from-gray-300 to-gray-400 text-white' :
                       ($index === 2 ? 'bg-gradient-to-br from-orange-300 to-orange-400 text-white' : 'bg-gray-100 text-gray-500')) }}">
                    {{ $index < 3 ? ['🥇','🥈','🥉'][$index] : '#'.($index+1) }}
                </div>
                <div class="flex-1 min-w-0">
                    <div class="font-bold text-sm text-gray-800 truncate group-hover:text-[#22AF85] transition-colors">{{ $tech['name'] }}</div>
                    <div class="text-[10px] text-gray-400 font-medium">{{ $tech['specialization'] }}</div>
                </div>
                <div class="text-right flex-shrink-0">
                    <div class="text-xl font-black text-[#22AF85]">{{ $tech['count'] }}</div>
