...
<section class="grid grid-cols-1 lg:grid-cols-2 gap-6 animate-fade-in-up delay-500">
    ...
    <div class="bg-white rounded-3xl p-6 shadow-xl border border-gray-100" x-data="{ tab: 'overdue' }">
        ...
        <div class="flex items-center gap-1 bg-gray-100 p-1 rounded-xl mb-4 w-fit">
            <button @click="tab='overdue'" :class="tab==='overdue' ? 'bg-white text-red-600 shadow-sm' : 'text-gray-400'" class="px-4 py-2 text-xs font-bold rounded-lg transition-all">
                🔥 Overdue (<span id="overdue-count">{{ $urgentActions['overdue_spks']->count() }}</span>)
            </button>
            <button @click="tab='stuck'" :class="tab==='stuck' ? 'bg-white text-orange-600 shadow-sm' : 'text-gray-400'" class="px-4 py-2 text-xs font-bold rounded-lg transition-all">
                ⏸️ Stuck (<span id="stuck-count">{{ $urgentActions['stuck_spks']->count() }}</span>)
            </button>
        </div>

        {{-- Overdue --}}
        <div x-show="tab==='overdue'" id="overdue-list" class="space-y-2 max-h-[350px] overflow-y-auto">
            @forelse($urgentActions['overdue_spks'] as $spk)
            <a href="{{ route('reception.show', $spk->id) }}" class="flex items-center justify-between p-3.5 rounded-2xl bg-red-50/50 border border-red-100 hover:bg-red-50 hover:border-red-200 transition-all group">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <div class="text-sm font-black text-gray-800 group-hover:text-red-600">{{ $spk->spk_number }}</div>
                        <div class="text-[10px] text-gray-400">{{ $spk->customer_name }}</div>
                    </div>
                </div>
                <div class="text-right">
                    <div class="text-xs font-bold text-red-500">{{ \Carbon\Carbon::parse($spk->estimation_date)->diffForHumans() }}</div>
                    <div class="text-[10px] text-gray-400">{{ $spk->status }}</div>
                </div>
            </a>
            @empty
        ...
        <div x-show="tab==='stuck'" x-cloak id="stuck-list" class="space-y-2 max-h-[350px] overflow-y-auto">
            @forelse($urgentActions['stuck_spks'] as $spk)
            <a href="{{ route('reception.show', $spk->id) }}" class="flex items-center justify-between p-3.5 rounded-2xl bg-orange-50/50 border border-orange-100 hover:bg-orange-50 hover:border-orange-200 transition-all group">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <div class="text-sm font-black text-gray-800 group-hover:text-orange-600">{{ $spk->spk_number }}</div>
                        <div class="text-[10px] text-gray-400">{{ $spk->customer_name }}</div>
                    </div>
                </div>
                <div class="text-right">
                    <div class="text-xs font-bold text-orange-500">Update {{ $spk->updated_at->diffForHumans() }}</div>
                    <div class="text-[10px] text-gray-400">{{ $spk->status }}</div>
                </div>
            </a>
            @empty
    ...
    {{-- Right: CX Issues + Low Stock + Quick Actions --}}
    <div class="space-y-6">
        {{-- CX Overdue --}}
        <div class="bg-white rounded-3xl p-6 shadow-xl border border-gray-100 {{ $urgentActions['cx_overdue']->count() > 0 ? 'border-l-4 border-l-orange-400' : '' }}">
            <div class="flex items-center justify-between mb-4">
                ...
                <span id="cx-overdue-count" class="px-2.5 py-1 bg-orange-100 text-orange-600 rounded-lg text-xs font-black" @if($urgentActions['cx_overdue']->count() == 0) style="display:none" @endif>{{ $urgentActions['cx_overdue']->count() }}</span>
            </div>
            <div id="cx-overdue-list" class="space-y-2 max-h-[200px] overflow-y-auto">
                @forelse($urgentActions['cx_overdue'] as $issue)
                <div class="flex items-center justify-between p-3 rounded-xl bg-orange-50/50 border border-orange-100">
                    <div class="flex items-center gap-2">
                        ...
                        <div>
                            <div class="text-xs font-bold text-gray-700">{{ $issue->workOrder->spk_number ?? $issue->spk_number ?? '-' }}</div>
                            <div class="text-[9px] text-gray-400 truncate max-w-[150px]">{{ $issue->category }}</div>
                        </div>
                    </div>
                    <div class="text-[10px] font-bold text-orange-500">{{ $issue->created_at->diffForHumans() }}</div>
                </div>
                @empty
        ...
        {{-- Low Stock --}}
        <div class="bg-white rounded-3xl p-6 shadow-xl border border-gray-100 {{ $urgentActions['low_stock']->count() > 0 ? 'border-l-4 border-l-red-400' : '' }}">
            ...
            <div id="low-stock-list" class="space-y-2 max-h-[150px] overflow-y-auto">
                @forelse($urgentActions['low_stock'] as $material)
                <div class="flex items-center justify-between p-3 rounded-xl bg-red-50/30 border border-red-100">
                    <div class="flex items-center gap-2">
                        ...
                        <div>
                            <div class="text-xs font-bold text-gray-700">{{ $material->name }}</div>
                            <div class="text-[9px] text-gray-400">Min: {{ $material->min_stock }} {{ $material->unit }}</div>
                        </div>
                    </div>
        ...
        <div class="grid grid-cols-2 gap-4">
            <a href="{{ route('reception.index') }}" class="group bg-gradient-to-br from-teal-500 to-teal-600 p-4 rounded-3xl shadow-lg border border-teal-400/30 hover:shadow-teal-500/20 transition-all text-center">
            ...
            <a href="{{ route('admin.customers.create') }}" class="group bg-white p-4 rounded-3xl shadow-lg border border-gray-100 hover:shadow-xl transition-all text-center">
