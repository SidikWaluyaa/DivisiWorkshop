{{-- Premium Header Section — matching CX/Workshop design --}}
        ...
        <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
            ...
            <div class="flex flex-col sm:flex-row gap-3 items-start sm:items-center">
                ...
                <div class="flex gap-1 bg-white/10 backdrop-blur-md rounded-xl p-1 border border-white/20">
                    ...
                    <button onclick="window.location.href='{{ route('dashboard', ['period' => $key]) }}'"
                        class="px-3 py-2 rounded-lg text-xs font-bold transition-all duration-200
                        {{ $selectedPeriod === $key ? 'bg-white text-teal-700 shadow-lg' : 'text-white/80 hover:bg-white/20' }}">
                        {{ $label }}
                    </button>
                    @endforeach
                </div>
                ...
                {{-- Date Picker --}}
                <form action="{{ route('dashboard') }}" method="GET" class="flex items-center gap-2 bg-white/15 backdrop-blur-md px-4 py-3 rounded-xl border border-white/20 shadow-lg">
        ...
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mt-8">
            <div class="bg-white/15 backdrop-blur-md rounded-2xl p-4 border border-white/20 hover:bg-white/25 transition-all">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <div id="hero-spk-active" class="text-2xl font-black text-white">{{ $kpi['workshop']['active'] }}</div>
            ...
            <div class="bg-white/15 backdrop-blur-md rounded-2xl p-4 border border-white/20 hover:bg-white/25 transition-all">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <div id="hero-leads" class="text-2xl font-black text-white">{{ $kpi['cs']['leads'] }}</div>
            ...
            <div class="bg-white/15 backdrop-blur-md rounded-2xl p-4 border border-white/20 hover:bg-white/25 transition-all">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <div id="hero-cx-open" class="text-2xl font-black text-white">{{ $kpi['cx']['open_issues'] }}</div>
            ...
            <div class="bg-white/15 backdrop-blur-md rounded-2xl p-4 border border-white/20 hover:bg-white/25 transition-all">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <div id="server-clock" class="text-2xl font-black text-white font-mono">{{ now()->format('H:i') }}</div>
                        <div class="text-[10px] text-white/60 font-bold uppercase tracking-wider">{{ \Carbon\Carbon::now()->translatedFormat('D, d M Y') }}</div>
                    </div>
                </div>
            </div>
        </div>
