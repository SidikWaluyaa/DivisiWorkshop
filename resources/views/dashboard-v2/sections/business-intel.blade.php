    ...
    <div class="lg:col-span-2 bg-white rounded-3xl p-6 shadow-xl border border-gray-100">
        <div class="flex items-center justify-between mb-6">
            ...
            <div class="px-4 py-2 bg-[#22AF85]/5 rounded-xl border border-[#22AF85]/10 text-right">
                ...
                <div class="text-xl font-black text-gray-800" id="revenue-total">Rp {{ number_format($businessIntel['revenue']['total'] / 1000, 0, ',', '.') }}rb</div>
            </div>
        </div>
        <div id="revenueChart" style="min-height: 300px;"></div>
    </div>

    {{-- Right Column: CX Analysis (1/3) --}}
    <div class="space-y-6">
        {{-- Top CX Issues --}}
        <div class="bg-white rounded-3xl p-6 shadow-xl border border-gray-100">
            ...
            <div id="top-issues-list" class="space-y-3">
                @forelse($businessIntel['topIssues'] as $issue)
                @php
                ...
                <div>
                    <div class="flex justify-between text-xs font-bold mb-1.5">
                        <span class="text-gray-600 truncate max-w-[160px]">{{ $issue->category ?? 'Uncategorized' }}</span>
                        <span class="text-[#22AF85] font-black">{{ $issue->count }}</span>
                    </div>
                    <div class="w-full bg-gray-100 rounded-full h-2.5">
                        <div class="bg-gradient-to-r from-[#22AF85] to-teal-400 h-2.5 rounded-full transition-all" style="width: {{ $pct }}%"></div>
                    </div>
                </div>
                @empty
        ...
        {{-- Complaint Summary --}}
        <div class="bg-white rounded-3xl p-6 shadow-xl border border-gray-100 {{ $businessIntel['complaints']['overdue'] > 0 ? 'border-l-4 border-l-red-400' : '' }}">
            ...
            <div class="grid grid-cols-3 gap-3">
                <div class="text-center p-3 bg-orange-50 rounded-xl border border-orange-100">
                    <div class="text-2xl font-black text-orange-500" id="complaint-pending">{{ $businessIntel['complaints']['pending'] }}</div>
                ...
                <div class="text-center p-3 bg-blue-50 rounded-xl border border-blue-100">
                    <div class="text-2xl font-black text-blue-500" id="complaint-process">{{ $businessIntel['complaints']['process'] }}</div>
                    <div class="text-[8px] text-blue-400 font-bold uppercase tracking-wider mt-1">Proses</div>
                </div>
                <div class="text-center p-3 {{ $businessIntel['complaints']['overdue'] > 0 ? 'bg-red-50 border border-red-200 urgent-glow' : 'bg-green-50 border border-green-100' }} rounded-xl">
                    <div class="text-2xl font-black {{ $businessIntel['complaints']['overdue'] > 0 ? 'text-red-500' : 'text-green-500' }}" id="complaint-overdue">{{ $businessIntel['complaints']['overdue'] }}</div>
                    <div class="text-[8px] {{ $businessIntel['complaints']['overdue'] > 0 ? 'text-red-400' : 'text-green-400' }} font-bold uppercase tracking-wider mt-1">Overdue</div>
                </div>
            </div>
        </div>
    </div>
