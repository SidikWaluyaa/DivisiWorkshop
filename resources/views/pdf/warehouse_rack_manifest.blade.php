...
<body>
        ...
        <table>
            <tr>
                <td width="33%">
                    ...
                    <div>{{ strtoupper(is_object($rack->category) ? ($rack->category->label() ?? $rack->category->value) : $rack->category) }}</div>
                </td>
                <td width="33%">
                    <div class="label">Kapasitas Terisi</div>
                    <div>{{ $rack->current_count }} / {{ $rack->capacity }} Item ({{ $rack->capacity > 0 ? round(($rack->current_count / $rack->capacity) * 100) : 0 }}%)</div>
                </td>
                <td width="33%">
                    <div class="label">Status Rak</div>
                    <div style="color: {{ (is_object($rack->status) ? $rack->status->value : $rack->status) == 'active' ? '#38a169' : '#e53e3e' }}">
                        {{ strtoupper(is_object($rack->status) ? ($rack->status->label() ?? $rack->status->value) : $rack->status) }}
                    </div>
                </td>
            </tr>
        </table>
    ...
    <div class="footer">
        SISTEM WORKSHOP - MANIFEST GUDANG UTAMA - ID RAK: {{ $rack->id }}
    </div>
</body>
</html>
