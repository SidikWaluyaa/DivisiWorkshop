<div class="p-6 space-y-4 bg-[#F8F9FA] min-h-screen relative font-sans">
        ...
        <div class="fixed bottom-4 right-6 left-auto z-50 w-[calc(100%-300px)] max-w-[1250px] bg-[#22AF85] p-3 rounded-2xl shadow-2xl flex items-center justify-between px-8 border border-white/10">
            <div class="flex items-center gap-10">
                <div class="text-white">
                    ...
                    @php $iCount = 0; foreach($spkGroups as $g) foreach($g['items'] as $i) $iCount += $i['quantity']; @endphp
                    <p class="text-2xl font-black tracking-tighter mt-1 leading-none">{{ $iCount }} Item</p>
                </div>
                <div class="w-px h-8 bg-white/20"></div>
                <div class="flex items-center gap-6 text-white/80">
                    <div class="text-center">
                        <span class="text-[8px] font-black uppercase tracking-widest block opacity-60">GRUP SPK</span>
    ...
    <div class="fixed inset-0 z-[60] flex items-center justify-center p-4">
        ...
        <div class="relative bg-white w-full max-w-2xl max-h-[85vh] rounded-3xl shadow-2xl flex flex-col overflow-hidden animate-in zoom-in-95 border border-gray-100">
            ...
            <!-- Modal Header -->
            <div class="p-5 border-b border-gray-100 flex items-center justify-between bg-gradient-to-r from-rose-600 to-red-700 text-white">
                <div class="flex items-center space-x-3">
                    ...
                    <div>
                        <h3 class="text-sm font-black uppercase tracking-wider">PILIH MATERIAL KELUAR</h3>
                        <p class="text-[10px] text-rose-100 mt-0.5">Pilih material berdasarkan Nama, Tipe, Size, dan Stok</p>
                    </div>
                </div>
            ...
            <div class="p-4 bg-gray-50/50 border-b border-gray-100">
                <div class="relative">
                    ...
                    <input type="text" wire:model.live.debounce.150ms="checklistSearch" 
                           ...
                           class="w-full pl-10 pr-10 py-2.5 bg-white border-gray-200 rounded-xl focus:border-rose-500 focus:ring-4 focus:ring-rose-500/10 font-bold text-xs text-gray-800 shadow-sm transition-all">
                    @if($checklistSearch)
            ...
            <div class="flex-1 overflow-y-auto p-4 space-y-2.5 custom-scrollbar">
                ...
                <div wire:click="toggleChecklist({{ $material->id }})"
                     ...
                     {{ $isSelected ? 'bg-rose-50 border-rose-500 shadow-md' : 'bg-white border-gray-100 hover:border-rose-200 hover:bg-rose-50/20' }}">
                    
                    <div class="flex items-center space-x-3.5 min-w-0">
                        <!-- Custom Checkbox Icon -->
                        <div class="w-6 h-6 rounded-lg flex items-center justify-center transition-all flex-shrink-0
                                    {{ $isSelected ? 'bg-rose-500 text-white shadow-sm scale-105' : 'bg-gray-100 group-hover:bg-gray-200 text-transparent' }}">
