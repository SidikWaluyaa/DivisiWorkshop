<div class="p-6 space-y-6 bg-[#F8F9FA] min-h-screen relative font-sans print:bg-white print:p-0">
    ...
    <div class="flex items-center justify-between max-w-[1000px] mx-auto print:hidden">
        <div class="flex items-center space-x-3">
            <a href="{{ route('storage.disbursement.index') }}" class="p-2 bg-white rounded-xl shadow-sm border border-gray-100 text-gray-400 hover:text-rose-500 transition-all">
        ...
        <div class="flex items-center gap-3">
            ...
            <a href="{{ route('storage.disbursement.edit', $disbursement->id) }}" class="px-6 py-2.5 bg-[#FFC232] text-gray-900 font-black text-[11px] rounded-xl shadow-lg hover:scale-105 transition-all uppercase tracking-widest">
                EDIT DATA
            </a>
        </div>
    </div>
    ...
    <div class="max-w-[1000px] mx-auto bg-white rounded-[2rem] shadow-xl border border-gray-100 overflow-hidden print:shadow-none print:border-none print:max-w-full">
                ...
                <div class="text-right">
                    <div class="bg-white p-4 rounded-2xl border border-gray-100 shadow-sm print:p-0 print:border-none print:shadow-none">
                        <div class="mb-4">
                            ...
                            <p class="text-lg font-black text-rose-500 leading-none mt-1">{{ $disbursement->disbursement_number }}</p>
                        </div>
                        <div class="grid grid-cols-2 gap-4 text-left">
                            <div>
                                <span class="text-[8px] font-black text-gray-300 uppercase tracking-widest block">TANGGAL OUT</span>
                                <p class="text-[11px] font-black text-gray-700 uppercase">{{ $disbursement->disbursement_date->format('d M Y') }}</p>
                            </div>
                            <div>
                                <span class="text-[8px] font-black text-gray-300 uppercase tracking-widest block">STATUS</span>
                                <p class="text-[11px] font-black text-green-500 uppercase">{{ $disbursement->status }}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="mt-6">
                        ...
                        <p class="text-sm font-black text-gray-900 uppercase">{{ $disbursement->external_reference ?: '-' }}</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="p-10 space-y-10 print:p-6 print:space-y-6">
            ...
            <div class="space-y-4">
                <div class="flex items-center justify-between border-b-2 border-gray-100 pb-2">
                    <div class="flex items-center gap-3">
                        ...
                        <h3 class="text-xs font-black text-gray-900 uppercase tracking-widest">DISTRIBUSI SPK: <span class="text-rose-500">{{ $group['spk_number'] ?: 'NON-SPK / UMUM' }}</span></h3>
                    </div>
                    <span class="text-[10px] font-bold text-gray-400 uppercase tracking-widest">{{ count($group['items']) }} Item Material</span>
                </div>
                    ...
                    <tbody class="divide-y divide-gray-50">
                        ...
                        <tr class="border-b border-gray-50 last:border-none">
                            ...
                            <td class="px-4 py-3 text-center">
                                <span class="text-sm font-black text-gray-900">{{ $item->quantity }} <small class="text-gray-400 font-bold uppercase ml-1">unit</small></span>
                            </td>
                            <td class="px-4 py-3 text-right">
                                <span class="text-sm font-black text-gray-400 italic">Rp {{ number_format($item->price, 0, ',', '.') }}</span>
                            </td>
                        </tr>
                        @endforeach
                    </tbody>
                ...
                <div class="w-full max-w-md space-y-3">
                    <div class="grid grid-cols-2 gap-3">
                        ...
                        <div class="flex justify-between items-center px-4 py-3 bg-gray-900 rounded-2xl shadow-lg text-white print:bg-white print:text-black print:border-2 print:border-black print:shadow-none">
                            ...
                            <span class="text-2xl font-black tracking-tighter">{{ $disbursement->items->sum('quantity') }}</span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="mt-8 p-6 bg-gray-50 rounded-2xl border border-dashed border-gray-200 print:p-4 print:bg-white">
                ...
                <p class="text-xs font-bold text-gray-600 leading-relaxed">"{{ $disbursement->notes }}"</p>
            </div>
            @endif
        ...
        <div class="p-6 text-center text-gray-300 print:p-2 print:text-gray-400">
            <p class="text-[8px] font-bold uppercase tracking-[0.4em]">Dokumen ini adalah bukti sah pengeluaran material workshop - {{ now()->format('d/m/Y H:i') }}</p>
        </div>
    </div>
</div>
