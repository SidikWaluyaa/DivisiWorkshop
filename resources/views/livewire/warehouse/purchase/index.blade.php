<div class="p-8 space-y-8 bg-[#FBFBFB] min-h-screen">
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <a href="{{ route('storage.purchase.create') }}" 
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div class="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm flex items-center space-x-4">
            ...
            <div>
                <div class="text-2xl font-black text-gray-900">{{ $purchases->total() }}</div>
        ...
        <div class="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm flex items-center space-x-4">
            ...
            <div>
                <div class="text-2xl font-black text-gray-900">{{ \App\Models\WarehousePurchase::where('status', 'PENDING')->count() }}</div>
        ...
        <div class="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm flex items-center space-x-4">
            ...
            <div>
                <div class="text-2xl font-black text-[#22AF85]">Rp {{ number_format(\App\Models\WarehousePurchase::where('status', 'COMPLETED')->sum('total_amount'), 0, ',', '.') }}</div>
                    ...
                    <tr class="group hover:bg-[#FBFBFB] transition-all">
                        <td class="px-8 py-6">
                            <div class="flex flex-col">
                                <span class="text-lg font-black text-gray-900 group-hover:text-[#22AF85] transition-colors tracking-tight">{{ $purchase->purchase_number }}</span>
                                <div class="flex items-center mt-1 space-x-2">
                                    <span class="text-xs font-bold text-gray-400">{{ $purchase->purchase_date->format('d M Y') }}</span>
                                    @if($purchase->external_reference)
                                        <span class="px-2 py-0.5 bg-gray-100 rounded text-[10px] font-bold text-gray-500">REF: {{ $purchase->external_reference }}</span>
                                    @endif
                                </div>
                            </div>
                        </td>
                        <td class="px-8 py-6">
                            <div class="flex flex-col items-center gap-2">
                                ...
                                <div class="text-[10px] font-black uppercase tracking-widest px-3 py-1 rounded-full
                                    {{ $purchase->purchase_type == 'Urgent' ? 'bg-red-50 text-red-500' : 
                        ...
                        <td class="px-8 py-6">
                            <div class="flex flex-wrap gap-1 max-w-[200px]">
                                @php $spks = $purchase->items->pluck('spk_number')->unique()->filter(); @endphp
                                @foreach($spks as $spk)
                                    <span class="text-[10px] font-black text-gray-400 bg-gray-50 px-2 py-0.5 rounded border border-gray-100 group-hover:border-[#22AF85]/30 transition-all">{{ $spk }}</span>
                                @endforeach
                                @if($spks->isEmpty()) - @endif
                            </div>
                        </td>
                        <td class="px-8 py-6">
                            <div class="text-lg font-black text-gray-900 tracking-tight">Rp {{ number_format($purchase->total_amount, 0, ',', '.') }}</div>
                            <div class="text-[10px] font-bold text-gray-400 uppercase tracking-widest">{{ $purchase->items->count() }} Item Material</div>
                        </td>
                        <td class="px-8 py-6 text-right">
                            <div class="flex justify-end items-center space-x-3 opacity-0 group-hover:opacity-100 transition-all">
                                <a href="{{ route('storage.purchase.show', $purchase->id) }}" 
                                ...
                                @if($purchase->status !== 'COMPLETED' && $purchase->status !== 'CANCELLED')
                                    <button wire:click="completePurchase({{ $purchase->id }})" 
                                ...
                                @endif
                                <a href="{{ route('storage.purchase.edit', $purchase->id) }}" 
