        ...
        <div class="flex items-center gap-3">
            <div class="bg-white px-5 py-2 rounded-2xl border border-gray-100 shadow-sm flex items-center gap-4">
                <div class="text-right">
                    ...
                    <p class="text-sm font-black text-gray-900 leading-none mt-1">{{ $transactions->total() }}</p>
                </div>
    ...
    <div class="max-w-[1600px] mx-auto bg-white p-6 rounded-[2rem] shadow-sm border border-gray-100 grid grid-cols-1 md:grid-cols-5 gap-6 print:hidden">
        ...
        <div class="space-y-1.5">
            ...
            <div class="grid grid-cols-3 gap-1 bg-gray-50 p-1 rounded-xl">
                <button wire:click="$set('type', '')" class="py-1.5 rounded-lg text-[8px] font-black uppercase tracking-widest transition-all {{ $type == '' ? 'bg-white shadow-sm text-gray-900' : 'text-gray-400' }}">ALL</button>
                <button wire:click="$set('type', 'IN')" class="py-1.5 rounded-lg text-[8px] font-black uppercase tracking-widest transition-all {{ $type == 'IN' ? 'bg-[#22AF85] text-white' : 'text-gray-400' }}">IN</button>
                <button wire:click="$set('type', 'OUT')" class="py-1.5 rounded-lg text-[8px] font-black uppercase tracking-widest transition-all {{ $type == 'OUT' ? 'bg-rose-500 text-white' : 'text-gray-400' }}">OUT</button>
            </div>
        </div>
                    ...
                    <tr class="group hover:bg-[#FBFBFB] transition-all">
                        <td class="px-8 py-4">
                            <div class="flex items-center space-x-3">
                                <div class="w-1.5 h-8 {{ $trx->type == 'IN' ? 'bg-[#22AF85]' : 'bg-rose-500' }} rounded-full opacity-0 group-hover:opacity-100 transition-all"></div>
                                <div>
                                    <p class="text-xs font-black text-gray-900 tracking-tight leading-none uppercase">{{ $trx->created_at->translatedFormat('d M Y') }}</p>
                                    <p class="text-[9px] font-bold text-gray-400 uppercase tracking-widest mt-1.5">{{ $trx->created_at->format('H:i:s') }} WIB</p>
                                </div>
                            </div>
                        </td>
                        <td class="px-8 py-4">
                            <p class="text-sm font-black text-gray-800 uppercase tracking-tight leading-none">{{ $trx->material->name }}</p>
                            <p class="text-[9px] font-bold text-gray-400 uppercase tracking-[0.2em] mt-1.5 italic">{{ $trx->material->unit }}</p>
                        </td>
                        ...
                        <td class="px-8 py-4 text-center">
                            <div class="flex flex-col items-center">
                                <span class="text-sm font-black tracking-tight {{ $trx->type == 'IN' ? 'text-[#22AF85]' : 'text-rose-500' }}">
                        ...
                        <td class="px-8 py-4">
                            <div class="flex items-center space-x-3">
                                ...
                                <div>
                                    <p class="text-[10px] font-black text-gray-900 uppercase leading-none">{{ $trx->user->name ?? 'SYSTEM' }}</p>
