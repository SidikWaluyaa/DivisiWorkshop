        ...
        <div class="flex items-center justify-between mb-10">
            <div class="space-y-1">
                <nav class="flex items-center gap-2 text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2">
                    <a href="{{ route('material-requests.index') }}" wire:navigate class="hover:text-[#22AF85] transition-colors">Procurement</a>
            ...
            <div class="hidden md:flex items-center gap-4">
                <a href="{{ route('material-requests.index') }}" wire:navigate class="px-6 py-3 bg-white border border-gray-200 text-gray-500 text-sm font-bold rounded-2xl hover:bg-gray-50 transition-all">
                    Cancel
                </a>
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="grid grid-cols-2 gap-3 p-1.5 bg-gray-50 rounded-2xl border border-gray-100">
                            <button wire:click="$set('type', 'SHOPPING')" 
                                    class="py-3.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all {{ $type === 'SHOPPING' ? 'bg-white text-[#22AF85] shadow-sm ring-1 ring-gray-100' : 'text-gray-400 hover:text-gray-600' }}">
                                Shopping
                            </button>
                            <button wire:click="$set('type', 'PRODUCTION_PO')" 
                                    class="py-3.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all {{ $type === 'PRODUCTION_PO' ? 'bg-white text-[#22AF85] shadow-sm ring-1 ring-gray-100' : 'text-gray-400 hover:text-gray-600' }}">
                                Production PO
                            </button>
                        </div>
                        <p class="text-[10px] text-gray-400 font-bold italic">
                            {{ $type === 'SHOPPING' ? 'Sifatnya umum (stok gudang/belanja rutin).' : 'Spesifik untuk kebutuhan satu SPK/Order.' }}
                        </p>
                    </div>
                            ...
                            <div class="space-y-3 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
                                @forelse($pendingOrders as $order)
                                    <div wire:click="toggleSpk({{ $order->id }})" 
                                         class="group flex items-center justify-between p-4 rounded-2xl border transition-all cursor-pointer {{ in_array($order->id, $selectedSpks) ? 'bg-[#22AF85]/10 border-[#22AF85] shadow-sm' : 'bg-gray-50 border-gray-100 hover:border-gray-200' }}">
                                        <div class="flex items-center gap-4">
                                            <div class="w-5 h-5 rounded-md border-2 flex items-center justify-center transition-all {{ in_array($order->id, $selectedSpks) ? 'bg-[#22AF85] border-[#22AF85]' : 'bg-white border-gray-200 group-hover:border-gray-300' }}">
                                            ...
                                            <div class="flex flex-col">
                                                <span class="text-xs font-black {{ in_array($order->id, $selectedSpks) ? 'text-[#22AF85]' : 'text-gray-900' }}">SPK #{{ $order->spk_number }}</span>
                                                <div class="flex flex-col">
                                                    <span class="text-[9px] font-bold text-gray-400 uppercase tracking-tighter">{{ $order->customer_name }}</span>
                                                    {{-- Material List Preview --}}
                                                    <span class="text-[10px] font-semibold text-gray-600 mt-1.5 line-clamp-1">
                                                        <span class="text-[#22AF85]">Butuh:</span> {{ $order->materials->pluck('name')->join(', ') }}
                                                    </span>
                                                </div>
                                            </div>
                                        </div>
                    ...
                    <div class="p-10 border-b border-gray-50">
                        <div class="flex items-center justify-between mb-8">
                            ...
                            <span class="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] bg-gray-50 px-3 py-1.5 rounded-lg border border-gray-100">
                                {{ count($selectedItems) }} Materials Selected
                            </span>
                        </div>
                            ...
                            @if(!empty($materialResults))
                                <div class="absolute left-0 right-0 mt-3 bg-white rounded-3xl shadow-2xl border border-gray-100 z-50 overflow-hidden ring-1 ring-black/5">
                                    @foreach($materialResults as $material)
                                        <button wire:click="addItem({{ $material['id'] }})" 
                                            ...
                                            <div class="flex items-center gap-4">
                                                ...
                                                <div class="flex flex-col">
                                                    <span class="text-sm font-black text-gray-900 group-hover:text-[#22AF85] transition-colors">{{ $material['name'] }}</span>
                                            ...
                                            <tr class="hover:bg-gray-50 transition-colors group">
                                                <td class="px-10 py-6">
                                                    <div class="flex items-center gap-4">
                                                        ...
                                                        <span class="text-sm font-black text-gray-900 group-hover:text-[#22AF85]">{{ $item['name'] }}</span>
                                                    </div>
                                                </td>
                                                <td class="px-8 py-6">
                                                    <div class="flex items-center justify-center">
                                                        <div class="inline-flex items-center bg-gray-50 rounded-xl p-1 border border-gray-100 ring-1 ring-black/5">
                                                            <button wire:click="$set('selectedItems.{{ $index }}.quantity', {{ max(1, $item['quantity'] - 1) }})" class="w-8 h-8 rounded-lg hover:bg-white flex items-center justify-center text-gray-400 hover:text-gray-900 transition-all">
                                                            ...
                                                            <input type="number" wire:model.live="selectedItems.{{ $index }}.quantity" class="w-14 text-center border-none bg-transparent text-xs font-black text-gray-900 p-0 focus:ring-0">
                                                            <button wire:click="$set('selectedItems.{{ $index }}.quantity', {{ $item['quantity'] + 1 }})" class="w-8 h-8 rounded-lg hover:bg-white flex items-center justify-center text-gray-400 hover:text-gray-900 transition-all">
                                                                <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M12 4v16m8-8H4"/></svg>
                                                            </button>
                                                        </div>
                                                        <span class="ml-3 text-[10px] font-bold text-gray-400 uppercase tracking-widest">{{ $item['unit'] }}</span>
                                                    </div>
                                                </td>
                                                <td class="px-8 py-6 text-right">
                                                    <span class="text-sm font-bold text-gray-400 tabular-nums">Rp {{ number_format($item['price']) }}</span>
                                                </td>
                                                <td class="px-8 py-6 text-right">
                                                    <span class="text-sm font-black text-gray-900 tabular-nums">Rp {{ number_format($item['price'] * $item['quantity']) }}</span>
                                                </td>
                                                <td class="px-10 py-6 text-right">
                                                    <button wire:click="removeItem({{ $index }})" class="p-2 text-gray-300 hover:text-rose-500 transition-colors">
                        ...
                        <div class="px-10 py-8 bg-gray-50/50 border-t border-gray-100 flex flex-col md:flex-row items-center justify-between gap-6">
                            <div class="flex items-center gap-10">
                                <div class="flex flex-col">
                                    ...
                                    <span class="text-lg font-black text-gray-900">{{ collect($selectedItems)->sum('quantity') }} Items</span>
                                </div>
                                ...
                                <div class="flex flex-col text-right md:text-left">
                                    ...
                                    <span class="text-2xl font-black text-[#22AF85] tabular-nums">Rp {{ number_format(collect($selectedItems)->sum(fn($i) => $i['price'] * $i['quantity'])) }}</span>
                                </div>
                            </div>
