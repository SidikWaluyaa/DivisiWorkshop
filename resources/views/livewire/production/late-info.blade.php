                    ...
                    <table class="min-w-full border-collapse">
                        ...
                        <tbody class="divide-y divide-gray-50">
                            @forelse($orders as $order)
                                <tr class="group hover:bg-white transition-all cursor-default" wire:key="order-{{ $order->id }}">
                                    ...
                                    <td class="px-6 py-8 min-w-[220px]">
                                        <div class="relative group">
                                            <select wire:change="updateDescription({{ $order->id }}, $event.target.value)" 
                                            ...
                                            <div class="absolute right-4 top-1/2 -translate-y-1/2 pointer-events-none opacity-40" wire:loading.remove wire:target="updateDescription({{ $order->id }})">
                                            ...
                                            <div wire:loading wire:target="updateDescription({{ $order->id }})" class="absolute right-4 top-1/2 -translate-y-1/2">
                                    ...
                                    <td class="px-6 py-8">
                                        <div class="flex items-center gap-4">
                                            ...
                                            <div class="relative group/photo" x-data="{ uploading: false }" x-on:livewire-upload-start="uploading = true" x-on:livewire-upload-finish="uploading = false" x-on:livewire-upload-error="uploading = false">
                                                <input type="file" class="hidden" wire:model="photos.{{ $order->id }}" id="photo-upload-{{ $order->id }}" accept="image/*">
                                                
                                                @if($order->material_photo_url)
                                                    <button wire:click.prevent="deletePhoto({{ $order->id }})" 
                                                ...
                                                <label for="photo-upload-{{ $order->id }}" 
                                                    ...
                                                    @if($order->material_photo_url)
                                                        <div class="relative h-full w-full">
                                                            <img src="{{ $order->material_photo_url }}" class="h-full w-full object-cover transition-transform group-hover/photo:scale-110">
                                            ...
                                            <div class="flex flex-col gap-2.5">
                                                ...
                                                <div class="flex flex-col gap-1">
                                                    ...
                                                    <div class="relative flex items-center">
                                                        <input type="text"
                                                               wire:change="updateMaterialName({{ $order->id }}, $event.target.value)"
                                                        ...
                                                        <div wire:loading wire:target="updateMaterialName({{ $order->id }})" class="ml-2 absolute -right-6">
                                                ...
                                                <div class="flex flex-col gap-1">
                                                    ...
                                                    <div class="relative flex items-center">
                                                        <input type="date"
                                                               wire:change="updateMaterialArrivalDate({{ $order->id }}, $event.target.value)"
                                                               value="{{ $order->material_arrival_date ? $order->material_arrival_date->format('Y-m-d') : '' }}"
                                                        ...
                                                        <div wire:loading wire:target="updateMaterialArrivalDate({{ $order->id }})" class="ml-2 absolute -right-6">
                                                            <svg class="animate-spin h-3 w-3 text-blue-600" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
                                                        </div>
                                                    </div>
                                                </div>
                                    ...
                                    <td class="px-6 py-8">
                                        <div class="flex flex-col gap-1.5">
                                            ...
                                            <div class="relative flex items-center">
                                                <input type="date"
                                                       wire:change="updateNewEstimationDate({{ $order->id }}, $event.target.value)"
                                                ...
                                                <div wire:loading wire:target="updateNewEstimationDate({{ $order->id }})" class="ml-2 absolute -right-6">
