            ...
            <div class="inline-block align-bottom bg-white rounded-[3rem] text-left overflow-hidden shadow-[0_35px_60px_-15px_rgba(0,0,0,0.5)] transform transition-all sm:my-8 sm:align-middle sm:max-w-2xl w-full border border-gray-100">
                            ...
                            <label class="block relative group cursor-pointer">
                                ...
                                <div class="p-6 bg-white border-2 border-transparent rounded-3xl group-hover:border-[#1B8A68]/20 group-hover:bg-emerald-50/30 transition-all shadow-sm">
                                    <div class="flex items-center gap-6">
                                        ...
                                        <div>
                                            <div class="text-lg font-black text-gray-900 italic tracking-tight uppercase leading-none mb-1 group-hover:text-[#1B8A68] transition-colors">{{ $spk->spk_number }}</div>
                                            <div class="flex items-center gap-3">
                                                <span class="text-[10px] font-black text-gray-400 uppercase tracking-widest italic leading-none">{{ $spk->shoe_brand }} • {{ $spk->shoe_type }}</span>
                                                <span class="w-1 h-1 rounded-full bg-gray-200"></span>
                                                <span class="text-[10px] font-black text-[#1B8A68] italic tracking-tight uppercase leading-none">Rp {{ number_format($spk->total_transaksi, 0, ',', '.') }}</span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                ...
                <div class="px-10 py-8 bg-white border-t border-gray-100 flex items-center justify-between">
                    <div class="flex flex-col">
                        ...
                        <span class="text-2xl font-black text-[#1B8A68] italic tracking-tighter leading-none">{{ count($selectedSpks) }} SPK</span>
                    </div>
