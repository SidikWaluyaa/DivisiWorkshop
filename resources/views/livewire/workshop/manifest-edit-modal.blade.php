<div>
    ...
    @if(auth()->user()?->email === 'admin@workshop.com')
        <button type="button" 
            ...
            <span>⚡ Edit Muatan Manifest (Super Admin)</span>
        </button>
    @endif

    {{-- Edit Modal Teleported to Body --}}
    @if($showModal && $manifest)
        ...
        <div class="fixed inset-0 z-[999999] overflow-y-auto flex items-center justify-center p-4 sm:p-6 bg-slate-950/80 backdrop-blur-md animate-fadeIn font-sans">
            <div class="bg-white rounded-3xl overflow-hidden shadow-2xl border border-slate-100 max-w-4xl w-full max-h-[90vh] flex flex-col relative z-10">
                    ...
                    <div class="relative z-10 flex items-center gap-3.5">
                        ...
                        <div>
                            ...
                            <h2 class="text-xl font-black text-white tracking-tight leading-tight">
                                Edit Muatan Manifest #{{ $manifest->manifest_number }}
                            </h2>
                        </div>
                    </div>
                ...
                <div class="p-6 sm:p-7 space-y-6 overflow-y-auto flex-1">
                    ...
                    <div class="p-4 rounded-2xl bg-slate-50 border border-slate-200/80 space-y-3">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="datetime-local" wire:model="dispatched_at" class="w-full px-3 py-2 rounded-xl border border-slate-200 text-xs font-bold bg-white focus:ring-2 focus:ring-amber-400">
                            </div>
                            <div>
                                <label class="block text-[10px] font-black text-slate-500 uppercase tracking-wider mb-1">Catatan Logistik / Driver</label>
                                <input type="text" wire:model="notes" placeholder="Catatan driver, plat nomor, dll..." class="w-full px-3 py-2 rounded-xl border border-slate-200 text-xs font-bold bg-white focus:ring-2 focus:ring-amber-400">
                            </div>
                        </div>
                    ...
                    {{-- 2. Add New SPK to this Manifest --}}
                    <div class="p-4 rounded-2xl bg-teal-50/50 border border-teal-200/80 space-y-3">
                        <div class="text-xs font-black text-teal-900 uppercase tracking-wider flex items-center gap-2">
                            <span>➕</span> Tambahkan SPK Baru ke Manifest Ini
                        </div>
                        <div class="relative">
                            <input type="text" 
                                   wire:model.live.debounce.300ms="searchSpk" 
                                   placeholder="Ketik Nomor SPK / Nama Pelanggan / Brand Sepatu..." 
                                   class="w-full pl-4 pr-4 py-2.5 rounded-xl border border-teal-300 text-xs font-bold bg-white focus:ring-2 focus:ring-teal-500">
                        </div>

                        {{-- Search Results --}}
                        @if(!empty($searchSpk))
                            <div class="bg-white rounded-xl border border-teal-200 shadow-md divide-y divide-slate-100 max-h-48 overflow-y-auto">
                                @forelse($availableWorkOrders as $avWo)
                                    <div class="p-3 flex items-center justify-between hover:bg-teal-50/50 transition-colors">
                                        <div>
                                            ...
                                            <div class="text-[9px] font-bold text-teal-600">Status: {{ $avWo->status->value ?? $avWo->status }}</div>
                                        </div>
                                        <button type="button" 
                    ...
                    <div class="space-y-3">
                        <div class="flex items-center justify-between">
                            <h4 class="text-xs font-black text-slate-800 uppercase tracking-wider">
                                📦 Daftar Muatan SPK Manifest (Total: {{ $manifest->workOrders->count() }} Unit)
                            </h4>
                        </div>
                        ...
                        <div class="rounded-2xl border border-slate-200 overflow-hidden shadow-xs">
                            <table class="min-w-full divide-y divide-slate-100 text-left">
                                ...
                                <tbody class="divide-y divide-slate-100 text-xs bg-white">
                                    @forelse($manifest->workOrders as $idx => $wo)
                                        <tr class="hover:bg-slate-50 transition-colors">
                                            ...
                                            <td class="px-4 py-3">
                                                <span class="px-2.5 py-0.5 rounded-full text-[9px] font-black bg-slate-100 text-slate-700 border border-slate-200">
                                                    {{ $wo->status->value ?? $wo->status }}
                                                </span>
                                            </td>
                                            <td class="px-4 py-3 text-right">
                                                <button type="button" 
                                                        wire:click="removeSpk({{ $wo->id }})" 
                                                        wire:confirm="Keluarkan SPK '{{ $wo->spk_number }}' dari Manifest ini?" 
                ...
                <div class="p-5 bg-slate-50 border-t border-slate-100 flex justify-between items-center gap-3 shrink-0">
                    <button type="button" 
                            wire:click="deleteEmptyManifest" 
                            wire:confirm="PERINGATAN: Apakah Anda yakin ingin membatalkan & menghapus dokumen Manifest ini? Semua SPK (jika ada) akan dikembalikan ke antrian siap kirim." 
                        ...
                        <span>🗑️ Batalkan / Hapus Manifest Ini</span>
                    </button>
