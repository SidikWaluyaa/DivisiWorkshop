...
<div id="sidebar-nav-container" class="flex-1 px-2 overflow-y-auto sidebar-scroll pb-4 min-h-0" style="max-height: calc(100vh - 180px);">
    ...
    <div x-data="{ 
        ...
        <a href="{{ route('cs.after-photos') }}" 
           ...
           :class="sidebarCollapsed ? 'justify-center' : ''">
            ...
            <span x-show="!sidebarCollapsed" class="nav-item-text ml-3 flex-1">Galeri After Photo</span>
            <span x-show="sidebarCollapsed" class="absolute left-full ml-2 px-2 py-1 bg-gray-900 text-white text-xs rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap pointer-events-none z-50">Galeri After</span>
        </a>
        @endif
        ...
        <a href="{{ route('cs.forecasting.index') }}" 
           ...
           :class="sidebarCollapsed ? 'justify-center' : ''">
            ...
            <span x-show="!sidebarCollapsed" class="nav-item-text ml-3 flex-1 text-teal-400 font-bold">Forecasting</span>
            <span x-show="sidebarCollapsed" class="absolute left-full ml-2 px-2 py-1 bg-gray-900 text-teal-400 text-xs font-bold rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap pointer-events-none z-50">Forecasting</span>
        </a>
        @endif
        </div>
    </div>
    ...
    <div x-data="{ 
        ...
        <a href="{{ route('finance.waiting-payment') }}" 
           ...
           :class="sidebarCollapsed ? 'justify-center' : ''">
            ...
            <span x-show="!sidebarCollapsed" class="nav-item-text ml-3 text-emerald-400 font-bold">Waiting Payment</span>
            @php $financeWaitingCount = \App\Models\WorkOrder::where('status', 'WAITING_PAYMENT')->count(); @endphp
    ...
    <div x-data="{ 
        ...
        <a href="{{ route('cx.shipping-monitoring') }}" 
           ...
           :class="sidebarCollapsed ? 'justify-center' : ''">
            <svg class="nav-icon flex-shrink-0 text-blue-400 group-hover:text-blue-300 transition-colors" :class="sidebarCollapsed ? 'w-6 h-6' : 'w-5 h-5'" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 17a2 2 0 11-4 0 2 2 0 014 0zM19 17a2 2 0 11-4 0 2 2 0 014 0z"></path>
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16V6a1 1 0 00-1-1H4a1 1 0 00-1 1v10a1 1 0 001 1h1m8-1a1 1 0 01-1 1H9m4-1V8a1 1 0 011-1h2.586a1 1 0 01.707.293l3.414 3.414a1 1 0 01.293.707V16a1 1 0 01-1 1h-1m-6-1a1 1 0 001 1h1M5 17a2 2 0 104 0m-4 0a2 2 0 114 0m6 0a2 2 0 104 0m-4 0a2 2 0 114 0"></path>
            </svg>
        ...
        <a href="{{ route('cx.warranty-claims.index') }}" 
           ...
           :class="sidebarCollapsed ? 'justify-center' : ''">
            ...
            <span x-show="!sidebarCollapsed" class="nav-item-text ml-3 flex-1 text-teal-400 font-bold tracking-tight">Inbox Klaim Garansi</span>
            
            @php $pendingClaimsCount = \App\Models\WarrantyClaim::where('status', 'PENDING')->count(); @endphp
            @if($pendingClaimsCount > 0)
                <span x-show="!sidebarCollapsed" class="ml-2 py-0.5 px-2 rounded-full text-xs font-bold bg-amber-500 text-white shadow-sm animate-pulse">
                    {{ $pendingClaimsCount }}
                </span>
                <span x-show="sidebarCollapsed" class="absolute top-2 right-2 w-2.5 h-2.5 bg-amber-500 border border-white rounded-full animate-pulse"></span>
            @endif
    ...
    <div x-data="{ 
        ...
        <a href="{{ route('admin.promotions.index') }}" 
           class="nav-item {{ request()->routeIs('admin.promotions.*') ? 'active' : '' }} flex items-center px-3 py-3 rounded-lg group relative border border-amber-500/10 bg-amber-500/5 hover:bg-amber-500/20"
           :class="sidebarCollapsed ? 'justify-center' : ''">
            <svg class="nav-icon flex-shrink-0 text-amber-400" :class="sidebarCollapsed ? 'w-6 h-6' : 'w-5 h-5'" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 7h.01M6 20h12a2 2 0 002-2V8a2 2 0 00-2-2H6a2 2 0 00-2 2v10a2 2 0 002 2zM9 16l3-3 3 3m0-6h.01M12 10h.01M12 14h.01M16 14h.01M16 10h.01"></path>
            </svg>
            <span x-show="!sidebarCollapsed" class="nav-item-text ml-3 flex-1 font-bold text-amber-400">Manajemen Promo</span>
            <span x-show="sidebarCollapsed" class="absolute left-full ml-2 px-2 py-1 bg-gray-900 text-amber-400 text-xs font-bold rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap pointer-events-none z-50">Promo</span>
        </a>
        @endif
        ...
        <a href="{{ route('admin.activity-logs.index') }}" 
           ...
           :class="sidebarCollapsed ? 'justify-center' : ''">
            ...
            <span x-show="!sidebarCollapsed" class="nav-item-text ml-3 text-teal-400 font-bold">Log Aktivitas</span>
            <span x-show="sidebarCollapsed" class="absolute left-full ml-2 px-2 py-1 bg-gray-900 text-teal-400 text-xs font-bold rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap pointer-events-none z-50">Logs</span>
        </a>
        @endif
...
<div class="mt-auto border-t border-white/10 p-4 bg-black/10 backdrop-blur-md rounded-t-2xl mx-2 mb-2">
    <div x-show="!sidebarCollapsed" class="flex items-center gap-3">
        ...
        <div class="flex-1 min-w-0">
            <p class="truncate text-sm font-bold text-white mb-0 uppercase tracking-tight">{{ Auth::user()->name }}</p>
            <p class="text-[10px] text-teal-200 truncate uppercase font-bold tracking-widest opacity-80">{{ Auth::user()->role }}</p>
        </div>
        <form method="POST" action="{{ route('logout') }}" @submit.prevent="if(confirm('Logout dari sistem?')) $el.submit()">
            @csrf
