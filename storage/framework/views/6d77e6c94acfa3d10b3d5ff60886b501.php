<div class="relative">
     ...
     <?php $__env->slot('header', null, []); ?> 
        <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6 w-full">
            <h2 class="font-black text-xl text-white leading-tight flex items-center gap-4">
                ...
                <?php echo e(__('Pusat Kendali Gudang')); ?>
    ...
    <div class="py-6 bg-[#F8FAFC] min-h-screen" wire:poll.10s="refreshData" x-data="{ 
        activeTab: <?php if ((object) ('activeTab') instanceof \Livewire\WireDirective) : ?>window.Livewire.find('<?php echo e($__livewire->getId()); ?>').entangle('<?php echo e('activeTab'->value()); ?>')<?php echo e('activeTab'->hasModifier('live') ? '.live' : ''); ?><?php else : ?>window.Livewire.find('<?php echo e($__livewire->getId()); ?>').entangle('<?php echo e('activeTab'); ?>')<?php endif; ?>.live,
        activeRackTab: <?php if ((object) ('activeRackTab') instanceof \Livewire\WireDirective) : ?>window.Livewire.find('<?php echo e($__livewire->getId()); ?>').entangle('<?php echo e('activeRackTab'->value()); ?>')<?php echo e('activeRackTab'->hasModifier('live') ? '.live' : ''); ?><?php else : ?>window.Livewire.find('<?php echo e($__livewire->getId()); ?>').entangle('<?php echo e('activeRackTab'); ?>')<?php endif; ?>.live,
            ...
            <div class="flex flex-col xl:flex-row justify-between items-center gap-4 bg-white/60 p-4 rounded-[1.5rem] shadow-lg border border-white glass-panel relative z-[50]">
                <div class="flex flex-col md:flex-row items-center gap-4 w-full xl:w-auto">
                    ...
                    <div class="flex items-center gap-1.5 p-1 bg-gray-50/80 rounded-[1.2rem] border border-gray-100 flex-wrap sm:flex-nowrap relative">
                        <button wire:click="$set('dateRange', 'today')" 
                                class="px-4 py-1.5 rounded-lg text-[9px] font-black transition-all uppercase tracking-tighter <?php echo e($dateRange === 'today' ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-400 hover:text-gray-600'); ?>">
                            HARI INI
                        </button>
                        <button wire:click="$set('dateRange', '7_days')" 
                                class="px-4 py-1.5 rounded-lg text-[9px] font-black transition-all uppercase tracking-tighter <?php echo e($dateRange === '7_days' ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-400 hover:text-gray-600'); ?>">
                            7 HARI
                        </button>
                        <button wire:click="$set('dateRange', '30_days')" 
                                class="px-4 py-1.5 rounded-lg text-[9px] font-black transition-all uppercase tracking-tighter <?php echo e($dateRange === '30_days' ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-400 hover:text-gray-600'); ?>">
                            30 HARI
                        </button>
                        ...
                        <button @click="showCustomDate = !showCustomDate" 
                                :class="showCustomDate || '<?php echo e($dateRange); ?>' === 'custom' ? 'bg-[#22AF85] text-white shadow-lg' : 'text-[#22AF85] hover:bg-[#22AF85]/5 bg-white/50 border border-white'"
                ...
                <div class="hidden xl:flex items-center gap-3 px-6 py-2.5 bg-[#22AF85]/5 border border-[#22AF85]/10 rounded-full">
                    ...
                    <span class="text-[10px] font-bold text-[#22AF85]/80 uppercase tracking-widest">
                        PERIODE: <span class="text-[#22AF85]"><?php echo e(\Carbon\Carbon::parse($startDate)->format('d M')); ?> - <?php echo e(\Carbon\Carbon::parse($endDate)->format('d M Y')); ?></span>
                    </span>
                </div>
            </div>

            
            <div x-show="showCustomDate || '<?php echo e($dateRange); ?>' === 'custom'" 
                ...
                <div class="grid grid-cols-1 xl:grid-cols-12 gap-6">
                        ...
                        <div class="relative z-10">
                            ...
                            <div class="grid grid-cols-2 lg:grid-cols-4 gap-x-6 gap-y-8">
                                ...
                                <div class="space-y-1">
                                    ...
                                    <div class="text-2xl font-black text-gray-900"><?php echo e($stats['pending_reception'] ?? 0); ?></div>
                                </div>
                                <div class="space-y-1 border-l border-gray-50 pl-6">
                                    <div class="text-[9px] font-black text-gray-400 uppercase tracking-widest flex items-center gap-1">
                                        <span>✨</span> DI FINISH
                                    </div>
                                    <div class="text-2xl font-black text-[#FFC232]"><?php echo e($stats['finished_not_stored'] ?? 0); ?></div>
                                </div>
                                <div class="space-y-1 border-l border-gray-50 pl-6">
                                    <div class="text-[9px] font-black text-gray-400 uppercase tracking-widest flex items-center gap-1">
                                        <span>📦</span> DI RAK
                                    </div>
                                    <div class="text-2xl font-black text-blue-600"><?php echo e($stats['stored_items'] ?? 0); ?></div>
                                </div>
                                <div class="space-y-1 border-l border-gray-50 pl-6">
                                    <div class="text-[9px] font-black text-gray-400 uppercase tracking-widest flex items-center gap-1">
                                        <span>🚀</span> SIAP DIAMBIL
                                    </div>
                                    <div class="text-2xl font-black text-[#22AF85]"><?php echo e($stats['ready_for_pickup'] ?? 0); ?></div>
                                </div>
                                ...
                                <div class="pt-4 border-t border-gray-50">
                                    ...
                                    <div class="text-2xl font-black text-gray-700"><?php echo e($stats['incoming_day'] ?? 0); ?></div>
                                ...
                                <div class="pt-4 border-t border-gray-50 border-l pl-6">
                                    ...
                                    <div class="text-2xl font-black text-gray-700"><?php echo e($stats['finished_day'] ?? 0); ?></div>
                                    <div class="text-[8px] font-bold text-gray-400 uppercase">Periode Ini</div>
                                </div>
                                <div class="pt-4 border-t border-gray-50 border-l pl-6">
                                    <div class="text-[9px] font-black text-gray-400 uppercase tracking-widest flex items-center gap-1">
                                    ...
                                    <div class="text-2xl font-black text-indigo-600"><?php echo e($stats['spk_print'] ?? 0); ?></div>
                                ...
                                <div class="pt-4 border-t border-gray-50 border-l pl-6">
                                    ...
                                    <div class="text-2xl font-black text-orange-500"><?php echo e($stats['shipping_pending'] ?? 0); ?></div>
                    ...
                    <div class="xl:col-span-4 grid grid-rows-2 gap-6">
                        <div class="bg-[#22AF85] rounded-[2rem] p-6 shadow-xl text-white relative overflow-hidden group">
                            ...
                            <div class="flex items-end justify-between mb-4">
                                <div class="text-5xl font-black text-white"><?php echo e($efficiencyStats['health_score'] ?? 0); ?>%</div>
                                <div class="px-3 py-1 bg-white/20 rounded-lg text-[9px] font-black uppercase text-white">SEMPURNA</div>
                            </div>
                            <div class="h-1.5 bg-white/20 rounded-full overflow-hidden">
                                <div class="h-full bg-[#FFC232] transition-all duration-1000" style="width: <?php echo e($efficiencyStats['health_score'] ?? 0); ?>%"></div>
                            </div>
                        </div>
                        ...
                        <div class="bg-white rounded-[2rem] p-6 shadow-lg border border-gray-100 relative group">
                            ...
                            <div class="flex items-center gap-4">
                                ...
                                <div>
                                    <div class="text-3xl font-black text-gray-900"><?php echo e($efficiencyStats['avg_dwell_hours'] ?? 0); ?><span class="text-base font-bold">jam</span></div>
                    ...
                    <div class="xl:col-span-12 bg-white rounded-[2rem] p-8 shadow-lg border border-gray-100">
                        ...
                        <div class="grid grid-cols-5 md:grid-cols-7 lg:grid-cols-10 xl:grid-cols-14 gap-3">
                            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::openLoop(); ?><?php endif; ?><?php $__currentLoopData = $heatmapData; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $rack): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::startLoop($loop->index); ?><?php endif; ?>
                                <div class="group relative" x-show="activeRackTab === '<?php echo e($rack['category']); ?>'" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 scale-95" x-transition:enter-end="opacity-100 scale-100">
                                    <div class="aspect-square rounded-xl border-2 transition-all duration-300 flex flex-col items-center justify-center cursor-help shadow-sm
                                        <?php echo e($rack['color'] === 'black' ? 'bg-gray-900 border-gray-900' : ($rack['color'] === 'yellow' ? 'bg-[#FFC232]/10 border-[#FFC232]/30' : 'bg-[#22AF85]/5 border-[#22AF85]/20')); ?>">
                                        <div class="text-[10px] font-black <?php echo e($rack['color'] === 'black' ? 'text-white' : ($rack['color'] === 'yellow' ? 'text-[#FFC232]' : 'text-[#22AF85]')); ?>"><?php echo e($rack['code']); ?></div>
                                        <div class="text-[8px] font-black <?php echo e($rack['color'] === 'black' ? 'text-white/40' : 'opacity-60'); ?>"><?php echo e($rack['count']); ?> Unit</div>
                                    </div>
                                    <div class="absolute bottom-[110%] left-1/2 -translate-x-1/2 mb-2 w-40 p-3 bg-gray-900 rounded-xl shadow-2xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-[100] text-white">
                                        <div class="flex justify-between items-center mb-2">
                                            <span class="text-[9px] font-black uppercase tracking-widest text-[#22AF85]">Rak <?php echo e($rack['code']); ?></span>
                                            <span class="text-[7px] bg-white/10 px-1.5 py-0.5 rounded text-white/50 opacity-100"><?php echo e($rack['category']); ?></span>
                                        </div>
                                        <div class="h-1 bg-white/10 rounded-full overflow-hidden">
                                            <div class="h-full bg-[#22AF85]" style="width: <?php echo e($rack['usage']); ?>%"></div>
                                        </div>
                                    </div>
                                </div>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div class="bg-white rounded-[2rem] overflow-hidden shadow-lg border border-gray-100">
                        <div class="px-6 py-4 border-b border-gray-50 flex items-center justify-between bg-gray-50/50">
                            ...
                            <span class="px-2 py-0.5 bg-[#22AF85] text-white text-[9px] font-black rounded-full"><?php echo e($queues['reception']->count()); ?></span>
                        </div>
                        <div class="divide-y divide-gray-50 max-h-[400px] overflow-y-auto sidebar-scroll">
                            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::openLoop(); ?><?php endif; ?><?php $__empty_1 = true; $__currentLoopData = $queues['reception']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $order): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::startLoop($loop->index); ?><?php endif; ?>
                                <div class="p-4 hover:bg-gray-50 transition-all flex items-center justify-between group">
                                    <div class="space-y-0.5">
                                        <div class="text-[10px] font-black text-[#22AF85]"><?php echo e($order->spk_number); ?></div>
                                        <div class="text-xs font-black text-gray-900"><?php echo e($order->customer_name); ?></div>
                                        <div class="text-[8px] text-gray-400 font-bold uppercase tracking-tighter">Antre <?php echo e($order->updated_at->diffForHumans(null, true)); ?></div>
                                    </div>
                                    <a href="<?php echo e(route('reception.show', $order->id)); ?>" class="opacity-0 group-hover:opacity-100 px-4 py-1.5 bg-[#FFC232] text-[9px] font-black rounded-lg transition-all uppercase shadow-lg shadow-[#FFC232]/20">
                                        Proses →
                                    </a>
                                </div>
                    ...
                    <div class="bg-white rounded-[2rem] overflow-hidden shadow-lg border border-gray-100">
                        <div class="px-6 py-4 border-b border-gray-50 flex items-center justify-between bg-gray-50/50">
                            ...
                            <span class="px-2 py-0.5 bg-[#FFC232] text-gray-900 text-[9px] font-black rounded-full"><?php echo e($queues['needs_qc']->count()); ?></span>
                        </div>
                        <div class="divide-y divide-gray-50 max-h-[400px] overflow-y-auto sidebar-scroll">
                            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::openLoop(); ?><?php endif; ?><?php $__empty_1 = true; $__currentLoopData = $queues['needs_qc']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $order): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::startLoop($loop->index); ?><?php endif; ?>
                                <div class="p-4 hover:bg-gray-50 transition-all flex items-center justify-between group">
                                    <div class="space-y-0.5">
                                        <div class="text-[10px] font-black text-[#22AF85]"><?php echo e($order->spk_number); ?></div>
                                        <div class="text-xs font-black text-gray-900"><?php echo e($order->customer_name); ?></div>
                                        <div class="text-[8px] text-gray-400 font-bold uppercase tracking-tighter">Tahap QC - <?php echo e($order->updated_at->diffForHumans(null, true)); ?></div>
                                    </div>
                                    <a href="<?php echo e(route('reception.show', $order->id)); ?>" class="opacity-0 group-hover:opacity-100 px-4 py-1.5 bg-[#22AF85] text-white text-[9px] font-black rounded-lg transition-all uppercase shadow-lg shadow-[#22AF85]/20">
                                        Inspeksi →
                                    </a>
                                </div>
                            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::endLoop(); ?><?php endif; ?><?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::closeLoop(); ?><?php endif; ?>
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        ...
                        <div class="bg-white rounded-[2rem] overflow-hidden shadow-lg border border-gray-100 border-t-4 border-t-red-500">
                            <div class="px-6 py-4 border-b border-gray-50 flex items-center justify-between bg-gray-50/50">
                                ...
                                <span class="px-2 py-0.5 bg-red-500 text-white text-[9px] font-black rounded-full"><?php echo e($queues['shipping_unverified']->count()); ?></span>
                            </div>
                            <div class="divide-y divide-gray-50 max-h-[300px] overflow-y-auto sidebar-scroll">
                                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::openLoop(); ?><?php endif; ?><?php $__empty_1 = true; $__currentLoopData = $queues['shipping_unverified']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $ship): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::startLoop($loop->index); ?><?php endif; ?>
                                    <div class="p-4 hover:bg-gray-50 transition-all flex items-center justify-between group">
                                        <div class="space-y-0.5">
                                            <div class="text-[10px] font-black text-red-500"><?php echo e($ship->spk_number); ?></div>
                                            <div class="text-xs font-black text-gray-900"><?php echo e($ship->customer_name); ?></div>
                                            <div class="text-[8px] text-gray-400 font-bold uppercase tracking-tighter"><?php echo e($ship->kategori_pengiriman); ?> • Menunggu Verifikasi</div>
                                        </div>
                        ...
                        <div class="bg-white rounded-[2rem] overflow-hidden shadow-lg border border-gray-100 border-t-4 border-t-[#22AF85]">
                            <div class="px-6 py-4 border-b border-gray-50 flex items-center justify-between bg-gray-50/50">
                                ...
                                <span class="px-2 py-0.5 bg-[#22AF85] text-white text-[9px] font-black rounded-full"><?php echo e($queues['shipping_verified']->count()); ?></span>
                            </div>
                            <div class="divide-y divide-gray-50 max-h-[300px] overflow-y-auto sidebar-scroll">
                                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::openLoop(); ?><?php endif; ?><?php $__empty_1 = true; $__currentLoopData = $queues['shipping_verified']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $ship): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::startLoop($loop->index); ?><?php endif; ?>
                                    <div class="p-4 hover:bg-gray-50 transition-all flex items-center justify-between group">
                                        <div class="space-y-0.5">
                                            <div class="text-[10px] font-black text-[#22AF85]"><?php echo e($ship->spk_number); ?></div>
                                            <div class="text-xs font-black text-gray-900"><?php echo e($ship->customer_name); ?></div>
                                            <div class="text-[8px] text-gray-500 font-bold uppercase tracking-tighter">
                                                <?php echo e($ship->resi_pengiriman ?? 'Resi Belum Input'); ?> • <?php echo e($ship->tanggal_pengiriman ? $ship->tanggal_pengiriman->format('d/m/Y') : 'Siap Kirim'); ?>
    ...
    <script>
        document.addEventListener('livewire:initialized', () => {
            ...
            const processCharts = (payload = null) => {
                // QC Trends
                const trendsData = payload ? payload.qcTrends : <?php echo json_encode($qcTrends, 15, 512) ?>;
                if (trendsData) {
                ...
                // QC Stats (Doughnut)
                const statsData = payload ? payload.qcStats : <?php echo json_encode($qcStats, 15, 512) ?>;
                if (statsData) {
                ...
                // Efficiency (Bar)
                const effData = payload ? payload.efficiency : <?php echo json_encode($efficiencyStats, 15, 512) ?>;
                if (effData) {
...
<?php /**PATH C:\laragon\www\SistemWorkshop\resources\views\livewire\warehouse\dashboard.blade.php ENDPATH**/ ?>