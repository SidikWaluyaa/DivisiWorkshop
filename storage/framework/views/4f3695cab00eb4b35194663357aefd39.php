                ...
                <div class="flex flex-col gap-5 sm:gap-8">
                    <div class="flex items-center justify-between">
                        ...
                        <div class="flex bg-gray-100 p-1 rounded-2xl border border-gray-200 shadow-inner">
                            <a href="<?php echo e(route('finance.cs-verification')); ?>" class="px-6 py-2.5 text-xs font-black uppercase tracking-widest bg-white text-orange-600 rounded-xl shadow-md border border-orange-100 italic">Pending</a>
                            <a href="<?php echo e(route('finance.cs-verification.history')); ?>" class="px-6 py-2.5 text-xs font-black uppercase tracking-widest text-gray-400 hover:text-gray-900 transition-all italic">History</a>
                        </div>
                    </div>

                    
                    <form action="<?php echo e(route('finance.cs-verification')); ?>" method="GET" class="flex items-center gap-3">
                        ...
                        <div class="flex flex-col lg:flex-row">
                            ...
                            <div class="lg:w-64 bg-gray-50 relative overflow-hidden group/img">
                                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($payment->proof_image): ?>
                                    <img src="<?php echo e(asset($payment->proof_image)); ?>" class="w-full h-full object-cover transition-transform duration-700 group-hover/img:scale-110">
                                    <div class="absolute inset-0 bg-black/40 opacity-0 group-hover/img:opacity-100 transition-opacity flex items-center justify-center">
                                        <button onclick="showProofLightbox('<?php echo e(asset($payment->proof_image)); ?>')" class="p-3 bg-white rounded-full text-gray-900 shadow-2xl transform translate-y-4 group-hover/img:translate-y-0 transition-transform">
                            ...
                            <div class="flex-1 p-8 lg:p-10 flex flex-col justify-between">
                                <div class="flex flex-col lg:flex-row lg:items-start justify-between gap-6 mb-8">
                                    <div>
                                        <div class="flex items-center gap-3 mb-3">
                                            <span class="px-2 py-1 bg-gray-900 text-white text-[9px] font-black rounded-lg uppercase tracking-widest italic"><?php echo e($payment->payment_method); ?></span>
                                            <span class="text-[10px] font-black text-gray-300 uppercase tracking-widest">TRX-<?php echo e($payment->id); ?></span>
                                        </div>
                                        <h2 class="text-3xl font-black text-gray-900 tracking-tighter italic uppercase leading-none mb-2">
                                            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(strtolower($payment->type) === 'before'): ?>
                                                DP dari CS
                                            <?php else: ?>
                                                Lunas di Awal
                                            <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                                        ...
                                        <div class="flex items-center gap-4 text-[11px] font-black text-gray-400 uppercase tracking-widest italic">
                                            <span>📅 <?php echo e($payment->paid_at->format('d M Y • H:i')); ?></span>
                                            <span class="text-gray-200">/</span>
                                            <span class="text-orange-500">PIC: <?php echo e($payment->pic->name ?? 'CS'); ?></span>
                                        </div>
                                    </div>

                                    <div class="text-left lg:text-right">
                                        <p class="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">NOMINAL SETORAN</p>
                                        <p class="text-4xl font-black text-gray-900 italic tracking-tighter leading-none">Rp <?php echo e(number_format($payment->amount_total, 0, ',', '.')); ?></p>
                                    </div>
                                </div>
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-3 gap-8 pt-8 border-t border-gray-50">
                                    <div class="space-y-1">
                                        ...
                                        <a href="<?php echo e(route('finance.show', $payment->work_order_id)); ?>" class="text-sm font-black text-[#1B8A68] hover:underline italic"><?php echo e($payment->spk_number_snapshot); ?></a>
                                    </div>
                                    <div class="space-y-1">
                                        <p class="text-[9px] font-black text-gray-400 uppercase tracking-widest">PELANGGAN</p>
                                        <p class="text-sm font-black text-gray-900 italic"><?php echo e($payment->customer_name_snapshot); ?></p>
                                    </div>
                                    <div class="space-y-1">
                            ...
                            <div class="lg:w-64 bg-gray-50 p-8 flex flex-col items-center justify-center border-l border-gray-100">
                                <button type="button" 
                                        @click="openVerifyModal(<?php echo e(json_encode($payment)); ?>)"
        ...
        <div x-show="showVerifyModal" 
            ...
            <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
                ...
                <div x-show="showVerifyModal" 
                    ...
                    <form :action="'<?php echo e(route('finance.verify-order-payment', ['id' => ':id'])); ?>'.replace(':id', selectedPayment.id)" method="POST">
                        <?php echo csrf_field(); ?>
...
<?php endif; ?>
<?php /**PATH C:\laragon\www\SistemWorkshop\resources\views/finance/cs_verification.blade.php ENDPATH**/ ?>