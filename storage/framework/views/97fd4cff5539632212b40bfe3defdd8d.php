...
<div id="editModal" class="hidden fixed inset-0 bg-gray-900/80 backdrop-blur-md overflow-y-auto h-full w-full z-[100] transition-all duration-300">
    <div class="relative top-10 mx-auto p-0 border-0 w-full max-w-2xl shadow-[0_20px_50px_rgba(0,0,0,0.3)] rounded-[2.5rem] bg-white mb-20 overflow-hidden">
        ...
        <div class="p-10">
            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(in_array($lead->status, ['CONVERTED', 'LOST'])): ?>
                <div class="mb-10 p-6 bg-red-50 border-2 border-red-100 rounded-[2rem] flex items-start gap-5">
                    ...
                    <div>
                        ...
                        <p class="text-gray-600 text-xs font-bold leading-relaxed">
                            Lead ini sudah berada di tahap <span class="bg-red-100 text-red-700 px-2 py-0.5 rounded-lg"><?php echo e($lead->status); ?></span>. Log audit akan mencatat setiap perubahan secara mendalam.
                        </p>
                    </div>
                </div>
            <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>

            <form action="<?php echo e(route('cs.leads.update', $lead->id)); ?>" method="POST" class="space-y-8">
                <?php echo csrf_field(); ?>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div class="space-y-6">
                        <div>
                            ...
                            <input type="text" name="customer_name" value="<?php echo e(old('customer_name', $lead->customer_name)); ?>" required 
                        ...
                        <div>
                            ...
                            <input type="text" name="customer_phone" value="<?php echo e(old('customer_phone', $lead->customer_phone)); ?>" required 
                                   class="w-full px-6 py-4 rounded-2xl border-2 border-gray-100 focus:border-[#22AF85] focus:ring-0 text-sm font-bold text-gray-900 transition-all bg-gray-50/30">
                        </div>
                        <div>
                            <label class="block text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] mb-3">Email</label>
                            <input type="email" name="customer_email" value="<?php echo e(old('customer_email', $lead->customer_email)); ?>" 
                                   class="w-full px-6 py-4 rounded-2xl border-2 border-gray-100 focus:border-[#22AF85] focus:ring-0 text-sm font-bold text-gray-900 transition-all bg-gray-50/30">
                        </div>
                    </div>
                    <div class="space-y-6">
                        <div>
                            ...
                            <select name="source" required class="w-full px-6 py-4 rounded-2xl border-2 border-gray-100 focus:border-[#22AF85] focus:ring-0 text-sm font-bold text-gray-900 transition-all bg-gray-50/30">
                                <option value="WhatsApp" <?php echo e($lead->source == 'WhatsApp' ? 'selected' : ''); ?>>WhatsApp</option>
                                <option value="Instagram" <?php echo e($lead->source == 'Instagram' ? 'selected' : ''); ?>>Instagram</option>
                                <option value="Website" <?php echo e($lead->source == 'Website' ? 'selected' : ''); ?>>Website</option>
                                <option value="Referral" <?php echo e($lead->source == 'Referral' ? 'selected' : ''); ?>>Referral</option>
                                <option value="Walk-in" <?php echo e($lead->source == 'Walk-in' ? 'selected' : ''); ?>>Walk-in</option>
                            </select>
                        </div>
                        <div>
                            ...
                            <select name="priority" required class="w-full px-6 py-4 rounded-2xl border-2 border-gray-100 focus:border-[#FFC232] focus:ring-0 text-sm font-bold text-gray-900 transition-all bg-gray-50/30">
                                <option value="HOT" <?php echo e($lead->priority == 'HOT' ? 'selected' : ''); ?>>🔥 HOT</option>
                                <option value="WARM" <?php echo e($lead->priority == 'WARM' ? 'selected' : ''); ?>>☀️ WARM</option>
                                <option value="COLD" <?php echo e($lead->priority == 'COLD' ? 'selected' : ''); ?>>❄️ COLD</option>
                            </select>
                        </div>
                    </div>
                </div>
                ...
                <div class="space-y-6">
                    <div>
                        ...
                        <textarea name="customer_address" rows="1" class="w-full px-6 py-4 rounded-2xl border-2 border-gray-100 focus:border-[#22AF85] focus:ring-0 text-sm font-bold text-gray-900 transition-all bg-gray-50/30"><?php echo e(old('customer_address', $lead->customer_address)); ?></textarea>
                    </div>
                    <div class="grid grid-cols-2 gap-8">
                        <div>
                            <label class="block text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] mb-3">Kota</label>
                            <input type="text" name="customer_city" value="<?php echo e(old('customer_city', $lead->customer_city)); ?>" 
                                   class="w-full px-6 py-4 rounded-2xl border-2 border-gray-100 focus:border-[#22AF85] focus:ring-0 text-sm font-bold text-gray-900 transition-all bg-gray-50/30">
                        </div>
                        <div>
                            <label class="block text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] mb-3">Provinsi</label>
                            <input type="text" name="customer_province" value="<?php echo e(old('customer_province', $lead->customer_province)); ?>" 
                                   class="w-full px-6 py-4 rounded-2xl border-2 border-gray-100 focus:border-[#22AF85] focus:ring-0 text-sm font-bold text-gray-900 transition-all bg-gray-50/30">
                        </div>
                    </div>
                </div>
                ...
                <div class="pt-8 border-t border-gray-100">
                    ...
                    <textarea name="revision_reason" rows="3" <?php echo e(in_array($lead->status, ['CONVERTED', 'LOST']) ? 'required' : ''); ?>
...
<?php /**PATH C:\laragon\www\SistemWorkshop\resources\views\cs\leads\partials\modals\edit-customer.blade.php ENDPATH**/ ?>