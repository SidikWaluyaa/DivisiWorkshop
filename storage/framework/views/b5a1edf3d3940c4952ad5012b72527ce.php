...
<?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::processComponentKey($component); ?>
                ...
                <div>
                    ...
                    <div class="flex items-center gap-3 mt-2">
                        <div class="bg-white/10 backdrop-blur-sm px-3 py-1 rounded-lg border border-white/20 flex items-center gap-2">
                            ...
                            <span class="text-xs font-black text-white uppercase tracking-widest"><?php echo e($order->spk_number); ?></span>
                        </div>
            ...
            <div class="flex flex-wrap items-center gap-4 w-full lg:w-auto">
                <div class="bg-white/10 backdrop-blur-sm border border-white/20 rounded-xl px-4 py-2.5 flex items-center gap-3 shadow-sm">
                    ...
                    <div class="flex flex-col">
                        ...
                        <span class="text-xs font-black text-white uppercase tracking-tight"><?php echo e($order->created_at->format('d M Y')); ?> <span class="text-[#FFC232] ml-1"><?php echo e($order->created_at->format('H:i')); ?></span></span>
                    </div>
                </div>
    ...
    <div class="py-8 bg-gradient-to-br from-gray-50 to-gray-100" x-data="assessmentServiceForm()">
        <div class="max-w-[95%] mx-auto sm:px-6 lg:px-8">
            <form action="<?php echo e(route('assessment.store', $order->id)); ?>" method="POST" id="assessmentForm">
                <?php echo csrf_field(); ?>
                    ...
                    <div class="col-span-12 lg:col-span-7">
                        <div class="bg-white overflow-hidden shadow-lg sm:rounded-2xl h-full flex flex-col border border-gray-100">
                            <div class="p-5 border-b bg-white flex justify-between items-center">
                                ...
                                <a href="<?php echo e(route('assessment.print-spk', $order->id)); ?>" target="_blank" class="inline-flex items-center gap-2 px-6 py-2.5 bg-[#FFC232] text-gray-900 font-black rounded-xl shadow-lg hover:shadow-xl hover:bg-[#FFC232]/90 transition-all transform hover:-translate-y-0.5 text-sm">
                                ...
                                <div x-data="photoGallery()">
                                    ...
                                    <div id="gallery-grid" class="grid grid-cols-4 gap-3">
                                        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::openLoop(); ?><?php endif; ?><?php $__currentLoopData = $order->photos; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $photo): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::startLoop($loop->index); ?><?php endif; ?>
                                            <div @click="handlePhotoClick('<?php echo e($photo->photo_url); ?>', <?php echo e($photo->id); ?>)" 
                                                 ...
                                                 :class="selectedPhotos.includes(<?php echo e($photo->id); ?>) ? 'border-[#22AF85] ring-4 ring-[#22AF85]/10' : 'border-gray-100 hover:border-[#22AF85]'">
                                                
                                                <img src="<?php echo e($photo->photo_url); ?>" class="w-full h-full object-cover transition-transform duration-300 group-hover:scale-110" :class="selectedPhotos.includes(<?php echo e($photo->id); ?>) ? 'opacity-70' : ''">
                                                ...
                                                <div x-show="isSelecting" class="absolute top-2 left-2 z-20">
                                                    <input type="checkbox" :value="<?php echo e($photo->id); ?>" x-model="selectedPhotos" @click.stop 
                                                ...
                                                <div x-show="!isSelecting" class="absolute top-2 right-2 flex flex-col gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity z-10">
                                                    ...
                                                    <button type="button" @click.stop="setAsCover(<?php echo e($photo->id); ?>)" 
                                                            class="bg-white/90 hover:bg-[#FFC232] text-gray-700 hover:text-black rounded-lg p-1.5 shadow-lg transition-all transform hover:scale-110 <?php echo e($photo->is_spk_cover ? 'hidden' : ''); ?>"
                                                    ...
                                                    <button type="button" @click.stop="deleteSingle(<?php echo e($photo->id); ?>, $el)" 
                                    ...
                                    <div id="empty-gallery-msg" class="text-center text-sm text-gray-400 py-8 italic <?php echo e($order->photos->count() > 0 ? 'hidden' : ''); ?>">Belum ada foto.</div>
                                        ...
                                        <div class="relative bg-white rounded-2xl shadow-2xl max-w-4xl max-h-[90vh] w-full flex flex-col overflow-hidden animate__animated animate__zoomIn animate__faster">
                                            <div class="flex items-center justify-between p-4 border-b bg-gray-50/50 backdrop-blur-sm">
                                                <div class="flex items-center gap-3">
                                                    ...
                                                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::openLoop(); ?><?php endif; ?><?php $__currentLoopData = $order->photos; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $p): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::startLoop($loop->index); ?><?php endif; ?>
                                                        <template x-if="currentPhotoId === <?php echo e($p->id); ?> && <?php echo e($p->is_spk_cover ? 'true' : 'false'); ?>">
                        ...
                        <div class="bg-white overflow-hidden shadow-lg sm:rounded-2xl border border-gray-100">
                            ...
                            <div class="p-6 space-y-5">
                                <div class="border-b pb-5">
                                    ...
                                    <div class="grid grid-cols-2 gap-4">
                                        <div>
                                            ...
                                            <input type="text" value="<?php echo e($order->customer_name); ?>" class="w-full px-3 py-2 bg-gray-50 border rounded-lg text-sm" readonly>
                                        </div>
                                        <div>
                                            <label class="block text-xs font-bold text-gray-600 mb-1.5">WhatsApp</label>
                                            <input type="text" value="<?php echo e($order->customer_phone); ?>" class="w-full px-3 py-2 bg-gray-50 border rounded-lg text-sm" readonly>
                                        </div>
                                        <div>
                                            <label class="block text-xs font-bold text-gray-600 mb-1.5">Email</label>
                                            <input type="text" value="<?php echo e($order->customer_email); ?>" class="w-full px-3 py-2 bg-gray-50 border rounded-lg text-sm" readonly>
                                        </div>
                                        <div class="col-span-2">
                                            ...
                                            <div class="w-full px-3 py-2 border border-[#22AF85]/20 rounded-lg text-sm bg-gray-50 text-gray-700 font-medium leading-relaxed">
                                                <div class="font-bold mb-1"><?php echo e($order->customer->address ?? '-'); ?></div>
                                                <div class="grid grid-cols-2 gap-x-4 gap-y-1 text-[11px] opacity-80 uppercase tracking-tight">
                                                    <div><span class="font-normal lowercase italic text-gray-400 mr-2">kel:</span> <?php echo e($order->customer->village ?? '-'); ?></div>
                                                    <div><span class="font-normal lowercase italic text-gray-400 mr-2">kec:</span> <?php echo e($order->customer->district ?? '-'); ?></div>
                                                    <div><span class="font-normal lowercase italic text-gray-400 mr-2">kota:</span> <?php echo e($order->customer->city ?? '-'); ?></div>
                                                    <div><span class="font-normal lowercase italic text-gray-400 mr-2">prov:</span> <?php echo e($order->customer->province ?? '-'); ?></div>
                                                    <div><span class="font-normal lowercase italic text-gray-400 mr-2">pos:</span> <?php echo e($order->customer->postal_code ?? '-'); ?></div>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <div class="border-b pb-5">
                                    ...
                                    <div class="grid grid-cols-2 gap-4">
                                        <div>
                                            ...
                                            <input type="text" value="<?php echo e($order->shoe_brand); ?>" class="w-full px-3 py-2 bg-gray-50 border rounded-lg text-sm" readonly>
                                        </div>
                                        <div>
                                            <label class="block text-xs font-bold text-gray-600 mb-1.5">Tipe</label>
                                            <input type="text" value="<?php echo e($order->shoe_type); ?>" class="w-full px-3 py-2 bg-gray-50 border rounded-lg text-sm" readonly>
                                        </div>
                                        <div>
                                            <label class="block text-xs font-bold text-gray-600 mb-1.5">Size</label>
                                            <input type="text" value="<?php echo e($order->shoe_size); ?>" class="w-full px-3 py-2 bg-gray-50 border rounded-lg text-sm" readonly>
                                        </div>
                                        <div>
                                            <label class="block text-xs font-bold text-gray-600 mb-1.5">Warna</label>
                                            <input type="text" value="<?php echo e($order->shoe_color); ?>" class="w-full px-3 py-2 bg-gray-50 border rounded-lg text-sm" readonly>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="border-b pb-5">
                                    ...
                                    <div class="grid grid-cols-2 lg:grid-cols-3 gap-4 mb-4">
                                        ...
                                        <div>
                                            ...
                                            <div class="inline-flex items-center gap-2 px-3 py-2 bg-orange-50 border border-orange-200 rounded-lg text-orange-600 font-black text-xs uppercase tracking-widest shadow-sm">
                                                ...
                                                <?php echo e($inboundRack->rack_code); ?>
                                    ...
                                    <div class="grid grid-cols-2 gap-2">
                                        ...
                                        <div class="flex items-center justify-between px-2.5 py-1.5 bg-gray-50 border border-gray-200 rounded-lg">
                                            <span class="text-[10px] font-bold text-gray-600 uppercase"><?php echo e($label); ?></span>
                                            <div class="flex gap-1">
                                                ...
                                                ?>
                                                <span class="px-1.5 py-0.5 rounded text-[8px] font-black <?php echo e($isEmpty ? 'bg-red-100 text-red-600' : 'bg-gray-200 text-gray-400'); ?>">T</span>
                                                <span class="px-1.5 py-0.5 rounded text-[8px] font-black <?php echo e($isNempel ? 'bg-[#22AF85] text-white' : 'bg-gray-200 text-gray-400'); ?>">N</span>
                                                <span class="px-1.5 py-0.5 rounded text-[8px] font-black <?php echo e($isSimpan ? 'bg-[#FFC232] text-white' : 'bg-gray-200 text-gray-400'); ?>">S</span>
                                            </div>
                                        </div>
                                        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::endLoop(); ?><?php endif; ?><?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::closeLoop(); ?><?php endif; ?>

                                        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(!$order->accessories_tali && !$order->accessories_insole && !$order->accessories_box && !$order->accessories_other): ?>
                                        <div class="col-span-2 text-[10px] text-gray-400 italic">Tidak ada aksesoris.</div>
                                        <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                                    </div>

                                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(!empty($order->accessories_other)): ?>
                                    <div class="mt-3 p-2 bg-gray-50 border border-gray-200 rounded-lg">
                                        ...
                                        <div class="text-[11px] text-gray-700 font-bold leading-tight"><?php echo e($order->accessories_other); ?></div>
                                    </div>
                                    <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                                </div>
                                <div class="mb-4">
                                    ...
                                    <div class="w-full px-3 py-3 border border-[#22AF85]/20 rounded-lg text-sm bg-gray-50 text-gray-700 italic font-medium leading-relaxed">
                                        "<?php echo e($order->notes ?: 'Tidak ada catatan khusus.'); ?>"
                                    </div>
                                ...
                                <div>
                                    ...
                                    <textarea name="technician_notes" x-model="technician_notes" rows="4" class="w-full px-4 py-3 border-2 border-gray-100 rounded-lg text-sm focus:border-[#22AF85] focus:ring-4 focus:ring-[#22AF85]/10 font-bold bg-white" placeholder="Contoh: Hati-hati bagian heel counter rapuh..."><?php echo e($order->technician_notes); ?></textarea>
                                </div>
                                <div class="grid grid-cols-2 gap-4">
                                    <div class="col-span-2">
                                        <label class="block text-xs font-bold text-gray-600 mb-1.5">Prioritas</label>
                                        <input type="text" value="<?php echo e($order->priority); ?>" class="w-full px-3 py-2 bg-gray-50 border rounded-lg text-sm" readonly>
                                        <input type="hidden" name="priority" value="<?php echo e($order->priority); ?>">
                                    </div>
                                </div>
                            </div>
                        </div>
    ...
    <script>
        function assessmentServiceForm() {
            return {
                masterServices: <?php echo json_encode($services, 15, 512) ?>,
                selectedServices: [
                    <?php $__currentLoopData = $order->workOrderServices; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $wos): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    {
                        service_id: '<?php echo e($wos->service_id ?? "custom"); ?>',
                        name: <?php echo json_encode($wos->service?->name ?? $wos->custom_service_name ?? 'Service', 15, 512) ?>,
                        custom_name: <?php echo json_encode($wos->custom_service_name ?? $wos->service?->name ?? 'Service', 15, 512) ?>,
                        category: <?php echo json_encode($wos->category_name ?? $wos->service?->category ?? '-', 15, 512) ?>,
                        price: <?php echo e($wos->cost ?? 0); ?>,
                        details: <?php echo json_encode(is_array($wos->service_details) ? array_values(array_filter($wos->service_details)) : [], 15, 512) ?>
                    },
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                ],
                showServiceModal: false,
                serviceForm: { category: '', service_id: '', custom_name: '', price: 0, details: [], newDetail: '' },
                technician_notes: <?php echo json_encode($order->technician_notes ?? '', 15, 512) ?>,
                discount: <?php echo e((float)($order->discount ?? 0)); ?>,
                init() { 
...
<?php endif; ?>
<?php /**PATH C:\laragon\www\SistemWorkshop\resources\views\assessment\create.blade.php ENDPATH**/ ?>