                                ...
                                <div class="bg-slate-900 rounded-[40px] p-10 lg:p-14 text-white relative overflow-hidden shadow-2xl shadow-slate-900/40">
                                    ...
                                    <div class="grid grid-cols-1 md:grid-cols-3 gap-8 relative z-10">
                                        ...
                                        <div class="bg-white/5 p-8 rounded-[35px] border border-white/10 hover:bg-white/[0.07] transition-all">
                                            ...
                                            <p class="text-2xl font-black text-white"><?php echo e($lead->spk->expected_delivery_date ? \Carbon\Carbon::parse($lead->spk->expected_delivery_date)->format('d M Y') : 'BELUM DISET'); ?></p>
                                        </div>
                                    </div>
        ...
        <div class="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/80 backdrop-blur-xl">
            <div class="glass-panel bg-white/95 w-full max-w-2xl overflow-hidden shadow-2xl border-white">
                <div class="p-8 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                                    ...
                                    <div class="flex justify-between items-start mb-4">
                                        <div class="flex items-center gap-3">
                                            ...
                                            <div>
                                                ...
                                                <h5 class="text-sm font-black text-slate-800 uppercase mt-1"><?php echo e($svc->name); ?></h5>
                                            </div>
                                        </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-12 gap-6 items-start mt-4 bg-slate-50/30 p-8 rounded-[35px] border border-slate-100/50">
                        
                        <div class="md:col-span-5">
                            <label class="text-[10px] font-black uppercase text-slate-400 mb-3 block tracking-[0.2em] px-2">Catatan Tambahan</label>
                            <div class="group relative">
                                <input type="text" wire:model.live.debounce.150ms="editingData.extra_notes" placeholder="Catatan manual (cth: Prioritas)..." class="w-full bg-white border border-slate-100 rounded-2xl p-5 text-sm font-bold focus:ring-4 focus:ring-emerald-500/10 focus:border-emerald-200 transition-all shadow-sm">
                                <div class="absolute right-4 top-1/2 -translate-y-1/2 text-slate-300 group-focus-within:text-emerald-400 transition-colors">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                                </div>
                            </div>
                        ...
                        <div class="md:col-span-4">
                            ...
                            <div class="flex items-center gap-3 bg-white rounded-2xl p-2 pr-5 border border-slate-100 shadow-sm focus-within:border-emerald-200 focus-within:ring-4 focus-within:ring-emerald-500/5 transition-all">
                                ...
                                <button wire:click="toggleEditingWarranty" type="button" class="relative inline-flex h-10 w-16 shrink-0 cursor-pointer rounded-xl border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none <?php echo e($editingData['is_warranty'] ? 'bg-emerald-500' : 'bg-slate-100'); ?>">
                                    <span class="pointer-events-none inline-block h-6 w-6 transform rounded-lg bg-white shadow ring-0 transition duration-200 ease-in-out <?php echo e($editingData['is_warranty'] ? 'translate-x-7 mt-1.5' : 'translate-x-1 mt-1.5'); ?>"></span>
                                </button>
                                <input type="text" wire:model.live.debounce.150ms="editingData.warranty_label" placeholder="Cth: 30 Hari" class="flex-grow bg-transparent border-0 p-0 text-sm font-black text-slate-700 focus:ring-0 placeholder:text-slate-300">
                            </div>
                        </div>
                        ...
                        <div class="md:col-span-3">
                            <label class="text-[10px] font-black uppercase text-slate-400 mb-3 block tracking-[0.2em] px-2">Ringkasan Item</label>
                            <div class="bg-slate-900/5 backdrop-blur-sm border border-slate-100 rounded-2xl p-4 min-h-[58px] flex items-center shadow-inner">
                                <p class="text-[11px] font-black text-emerald-600 italic leading-snug">
                                    <?php echo e($this->calculateEditingSummary() ?: 'Pilih jasa untuk melihat ringkasan'); ?>
...
unset($__errorArgs, $__bag); ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                                    ...
                                    <div>
                                        <label class="text-[10px] font-black uppercase text-slate-500 mb-2 block tracking-widest">Estimasi Selesai</label>
                                        <input type="date" wire:model="spkData.expected_delivery_date" class="w-full bg-slate-50 border-0 rounded-2xl p-5 text-sm font-black focus:ring-4 focus:ring-emerald-500/10 shadow-sm <?php $__errorArgs = ['spkData.expected_delivery_date'];
$__bag = $errors->getBag($__errorArgs[1] ?? 'default');
if ($__bag->has($__errorArgs[0])) :
if (isset($message)) { $__messageOriginal = $message; }
$message = $__bag->first($__errorArgs[0]); ?> ring-2 ring-red-500 <?php unset($message);
...
unset($__errorArgs, $__bag); ?>">
                                        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php $__errorArgs = ['spkData.expected_delivery_date'];
$__bag = $errors->getBag($__errorArgs[1] ?? 'default');
if ($__bag->has($__errorArgs[0])) :
if (isset($message)) { $__messageOriginal = $message; }
$message = $__bag->first($__errorArgs[0]); ?> <span class="text-[10px] text-red-500 font-bold mt-1 block"><?php echo e($message); ?></span> <?php unset($message);
