...
<script>
    ...
    function openEditItemModal(itemId, btn) {
        ...
        const isLocked = <?php echo json_encode(in_array($lead->status, ['CONVERTED', 'LOST'])) ?>;
    ...
    function moveToKonsultasi() {
        ...
        if (notes !== null) {
            fetch("<?php echo e(route('cs.leads.move-konsultasi', $lead->id)); ?>", {
                method: 'POST',
                headers: {
    ...
    function moveToClosing() {
        if (confirm('Pindahkan lead ke Closing?')) {
            fetch("<?php echo e(route('cs.leads.move-closing', $lead->id)); ?>", {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>'
    ...
    function moveToFollowUp() {
        if (confirm('Pindahkan lead ke Follow-up Konsultasi?')) {
            fetch("<?php echo e(route('cs.leads.move-follow-up', $lead->id)); ?>", {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>'
    ...
    function backToKonsultasi() {
        ...
        if (notes !== null) {
            fetch("<?php echo e(route('cs.leads.update-status', $lead->id)); ?>", {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>'
    ...
    function markLost() {
        ...
        if (reason !== null && reason.trim() !== '') {
            const form = document.createElement('form');
            form.method = 'POST';
            form.action = "<?php echo e(route('cs.leads.mark-lost', $lead->id)); ?>";
    ...
    function updateSpkPreview() {
        ...
        const dateStr = "<?php echo e(date('ym-d')); ?>";
...
<?php /**PATH C:\laragon\www\SistemWorkshop\resources\views\cs\leads\partials\scripts.blade.php ENDPATH**/ ?>