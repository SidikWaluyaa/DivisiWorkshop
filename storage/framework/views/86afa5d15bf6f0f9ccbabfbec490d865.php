<div class="p-6 space-y-4 bg-[#F8F9FA] min-h-screen relative font-sans">
    ...
    <div class="flex items-center justify-between max-w-[1600px] mx-auto">
        <div class="flex items-center space-x-3">
            <a href="<?php echo e(route('storage.disbursement.index')); ?>" class="p-1.5 bg-white rounded-lg shadow-sm border border-gray-100 text-gray-400 hover:text-rose-500 transition-all">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"></path></svg>
            </a>
            <div>
                <h1 class="text-xl font-black text-gray-900 leading-none tracking-tight"><?php echo e($disbursementId ? 'EDIT' : 'BARU'); ?> <span class="text-rose-500">BARANG KELUAR</span></h1>
        ...
        <div class="bg-white px-4 py-1.5 rounded-xl border border-gray-100 shadow-sm flex items-center gap-3">
            <div class="text-right">
                ...
                <p class="text-[11px] font-black text-rose-500 leading-none mt-1"><?php echo e($disbursement_number); ?></p>
            </div>
            <div class="w-px h-6 bg-gray-100"></div>
            <div class="flex items-center gap-1.5">
                <div class="w-1.5 h-1.5 rounded-full <?php echo e($status === 'COMPLETED' ? 'bg-green-500' : 'bg-rose-500'); ?>"></div>
            ...
            <div class="space-y-1">
                ...
                <div class="grid grid-cols-2 gap-2">
                    <button type="button" wire:click="$set('status', 'PENDING')"
                            ...
                            <?php echo e($status == 'PENDING' ? 'bg-gray-100 border-gray-200 text-gray-600' : 'bg-white border-gray-50 text-gray-300'); ?>">
                        REQUEST
                    </button>
                    <button type="button" wire:click="$set('status', 'COMPLETED')"
                            class="py-2 rounded-lg font-black text-[8px] uppercase tracking-widest transition-all border
                            <?php echo e($status == 'COMPLETED' ? 'bg-green-50 border-green-200 text-green-600' : 'bg-white border-gray-50 text-gray-300'); ?>">
                        SELESAI
                    </button>
                </div>
            </div>
                    ...
                    <table class="w-full text-left">
                        ...
                        <tbody class="divide-y divide-gray-50">
                            ...
                            <tr class="hover:bg-gray-50/50 transition-all">
                                <td class="py-2 text-center font-black text-gray-200"><?php echo e($iIndex + 1); ?></td>
                                <td class="py-2 pl-2">
                                    <p class="font-black text-gray-700 uppercase leading-none"><?php echo e($item['material_name']); ?></p>
                                    <p class="text-[8px] font-bold text-gray-400 uppercase tracking-widest mt-1">STOK: <?php echo e($item['material_stock']); ?></p>
                                </td>
                                <td class="py-2">
                                    ...
                                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php $__errorArgs = ["spkGroups.{$gIndex}.items.{$iIndex}.quantity"];
$__bag = $errors->getBag($__errorArgs[1] ?? 'default');
if ($__bag->has($__errorArgs[0])) :
if (isset($message)) { $__messageOriginal = $message; }
$message = $__bag->first($__errorArgs[0]); ?> <p class="text-[7px] text-red-500 font-bold mt-1 text-center">OVER</p> <?php unset($message);
        ...
        <div class="fixed bottom-4 right-6 left-auto z-50 w-[calc(100%-300px)] max-w-[1250px] bg-[#22AF85] p-3 rounded-2xl shadow-2xl flex items-center justify-between px-8 border border-white/10">
            <div class="flex items-center gap-10">
                <div class="text-white">
                    ...
                    <p class="text-2xl font-black tracking-tighter mt-1 leading-none"><?php echo e($iCount); ?> Material</p>
                </div>
                <div class="w-px h-8 bg-white/20"></div>
                <div>
                    <span class="text-[8px] font-black text-white/60 uppercase tracking-widest block leading-none">STATUS</span>
                    <p class="text-sm font-black text-white mt-1 uppercase"><?php echo e($status); ?></p>
                </div>
            </div>
        ...
        <div class="relative bg-white w-full max-w-lg max-h-[70vh] rounded-2xl shadow-2xl flex flex-col overflow-hidden animate-in zoom-in-95">
            ...
            <div class="flex-1 overflow-y-auto p-4 space-y-1.5 custom-scrollbar">
                ...
                <button type="button" wire:click="toggleChecklist(<?php echo e($material->id); ?>)"
                        ...
                        <?php echo e(in_array($material->id, $selectedChecklist) ? 'bg-rose-50 border-rose-500 shadow-sm' : 'bg-white border-gray-100 hover:border-gray-200'); ?>">
                    <div class="flex items-center text-left">
                        <div class="w-6 h-6 rounded flex items-center justify-center mr-3 <?php echo e(in_array($material->id, $selectedChecklist) ? 'bg-rose-500 text-white' : 'bg-gray-100'); ?>">
                        ...
                        <div>
                            <span class="font-black text-gray-700 text-[11px] uppercase block leading-tight"><?php echo e($material->name); ?></span>
                            <span class="text-[8px] font-bold text-gray-400 uppercase tracking-widest mt-1">STOK: <?php echo e($material->stock); ?></span>
                        </div>
                    </div>
                </button>
                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::endLoop(); ?><?php endif; ?><?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::closeLoop(); ?><?php endif; ?>
...
<?php /**PATH C:\laragon\www\SistemWorkshop\resources\views/livewire/warehouse/disbursement/form.blade.php ENDPATH**/ ?>