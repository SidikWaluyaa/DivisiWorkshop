...
class ReceptionService
{
    ...
    public function confirmOrder(WorkOrder $order, ?string $manualRackCode = null): void
    {
        ...
        // -------------------------------------------------------------
        // AUTOMATED UNIVERSAL INVOICE GENERATION LOGIC
        // -------------------------------------------------------------
        if ($order->invoice_id === null) {
            DB::transaction(function () use ($order) {
                $customerName = $order->customer_name;
                $customerPhone = $order->customer_phone;
                $dateString = $order->created_at->toDateString();

                // Find all matching same-day, same-customer work orders that do not have an invoice yet
                $matchingOrders = WorkOrder::whereNull('invoice_id')
                    ->whereNotIn('status', [
                ...
                if ($matchingOrders->isNotEmpty()) {
                    ...
                    if (!$customerId) {
                        $normalizedPhone = \App\Helpers\PhoneHelper::normalize($customerPhone);
                        $customerId = \App\Models\Customer::where('phone', $normalizedPhone)->value('id');
                    }
                    ...
                    // Link all matching WorkOrders to this new Invoice
                    WorkOrder::whereIn('id', $matchingOrders->pluck('id'))->update(['invoice_id' => $invoice->id]);

                    // Sync existing payments to the new Invoice
                    foreach ($matchingOrders as $mo) {
                        $orderPayments = $mo->payments()->whereNull('invoice_id')->get();
                    ...
                    // Sync financials of the invoice (handles robust recalculations and URL shares generation)
                    $invoice->syncFinancials();

                    // -------------------------------------------------------------
                    // DEFERRED PROMO LOGGING LOGIC
                    // -------------------------------------------------------------
                    foreach ($matchingOrders as $mo) {
                        $spkItem = \App\Models\CsSpkItem::where('work_order_id', $mo->id)->first();
                        if ($spkItem && $spkItem->spk) {
                            $spk = $spkItem->spk;
                            $promoId = $spk->items()->whereNotNull('promotion_id')->value('promotion_id');
                            if ($promoId) {
                                $alreadyLogged = \App\Models\PromotionUsageLog::where('cs_spk_id', $spk->id)->exists();
                                if (!$alreadyLogged) {
                                    $promo = \App\Models\Promotion::find($promoId);
                                    if ($promo) {
                                        try {
                                            $originalAmount = (float)$spk->items()->sum('original_price');
                                            $discountAmount = (float)$spk->items()->sum('discount_amount');
                                            $finalAmount = (float)$spk->items()->sum('item_total_price');

                                            $promoService = app(\App\Services\PromoService::class);
                                            $promoService->logPromoUsage($promo, [
                                                'cs_lead_id' => $spk->cs_lead_id,
                                                'cs_spk_id' => $spk->id,
                                                'work_order_id' => $mo->id,
                                                'customer_phone' => $spk->lead->customer_phone ?: $mo->customer_phone,
                                                'original_amount' => $originalAmount,
                                                'discount_amount' => $discountAmount,
                                                'final_amount' => $finalAmount,
                                                'applied_by' => $spk->handed_by ?: Auth::id() ?: 1,
                                            ]);
                                            
                                            Log::info("Deferred Promo usage successfully logged for SPK #{$spk->spk_number} during Reception Confirmation.");
                                        } catch (\Throwable $e) {
                                            Log::error("Failed to log deferred promo usage during Reception Confirmation: " . $e->getMessage());
                                        }
                                    }
                                } else {
                                    // If already logged, ensure work_order_id is synced
                                    \App\Models\PromotionUsageLog::where('cs_spk_id', $spk->id)
                                        ->whereNull('work_order_id')
                                        ->update(['work_order_id' => $mo->id]);
                                }
                            }
                        }
                    }
                    
                    Log::info("Autogenerated Invoice {$invoiceNumber} created for " . count($matchingOrders) . " work orders.");
                }
            });
        }
    }
