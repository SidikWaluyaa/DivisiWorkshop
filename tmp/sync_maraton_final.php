...
echo "FINAL CALIBRATION v4 (GRANULAR ACCURACY): 1 - 4 APRIL 2026\n";
echo "===========================================================\n";
...
function syncPrecisionFinal($spkPart, $targetDate, $notes, $shouldCount = true) {
    ...
    // IMPORTANT: Set created_at to be way before service creation
    ...
    if (!$issue) {
        $issue = CxIssue::create([
            'work_order_id' => $wo->id,
            'description' => "Precision sync v4: " . $notes,
            'status' => 'RESOLVED',
...
syncPrecisionFinal('0307-QA', '2026-04-01', 'ganti alas jeruk', true);              // Akbar 0307 (225k) - EXACT MATCH
syncPrecisionFinal('0306-QA', '2026-04-01', 'GANTI ALAS POLOS', true);              // Akbar 0306 (225k) - As Amin Replacement

// Clean up others (Set to 'lanjut')
$others1 = ['0314-AW','0587-AW','0264-MY','0300-RQ','0197-VN','0819-AW','0436-VN','0305-QA'];
foreach($others1 as $s) syncPrecisionFinal($s, '2026-04-01', 'Penyisihan data v4', false);
...
syncPrecisionFinal('0394-AW', '2026-04-02', 'Midsole Non flat', true);              // Ryan (335k) - v4 STRICT MATCH
...
syncPrecisionFinal('0502-AW', '2026-04-03', 'Bemper', true);                        // Reinhard (225k)
syncPrecisionFinal('0900-RQ', '2026-04-03', 'Repaint spesial', true);               // Rony (175k) - v4 FIXED OVERCOUNT
